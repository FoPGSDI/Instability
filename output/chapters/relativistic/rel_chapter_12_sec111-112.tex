...
%  Relativistic framework: SR perfect fluid + Israel-Stewart dissipation
...
\subsection{Effect of viscosity: Israel--Stewart corrections to
capillary modes}
...
generalisation must employ the \emph{Israel--Stewart} causal
dissipative framework to avoid acausal signal propagation.

The Israel--Stewart shear-stress evolution in the jet rest frame
gives (cf.~\S\ref{sec:rel-IS})
\begin{equation}
\label{eq:rel-111-IS}
\taupi\,\frac{\partial\pi^{\mu\nu}}{\partial t}
  + \pi^{\mu\nu}
  = 2\,\shearvisc\,\sigma^{\mu\nu},
\end{equation}
where $\taupi$ is the shear relaxation time, $\shearvisc$ the shear
viscosity coefficient, and $\sigma^{\mu\nu}$ the fluid shear tensor.
Linearising and taking a normal mode $\sim e^{\sigma t}$, the
effective viscosity becomes \emph{frequency-dependent}:
\begin{equation}
\label{eq:rel-111-nueff}
\shearvisc_{\mathrm{eff}}(\sigma)
  = \frac{\shearvisc}{1 + \taupi\sigma}.
\end{equation}

The relativistic dispersion relation with Israel--Stewart
viscosity is obtained by replacing the Newtonian $\rho$ with
$\enthalpy$ and the kinematic viscosity $\nu$ with
$\nu_{\mathrm{eff}} = \shearvisc_{\mathrm{eff}}/\enthalpy$
in the classical dispersion relation~\eqref{eq:12-156}.
Define the relativistic capillary number
\begin{equation}
...
\qquad
\nu_{\mathrm{eff}}(\sigma)
  = \frac{\shearvisc}{\enthalpy(1+\taupi\sigma)}.
\end{equation}

\begin{relcorrection}
\textbf{Newtonian limit.}
Setting $\taupi \to 0$ (Navier--Stokes limit), $\enthalpy c^{2}
\to \rho c^{2}$, the effective viscosity becomes
$\nu_{\mathrm{eff}} \to \nu = \shearvisc/\rho$,
...
\sigma = \frac{T}{6\,\enthalpy c^{2}\,
  ...
  - (1+x^{2})^{-1}]}.
\end{equation}
In this limit the Israel--Stewart relaxation time $\taupi$ enters
through $\nu_{\mathrm{eff}}$; for $\sigma\taupi \ll 1$ one
recovers the Navier--Stokes result (with $\rho \to \enthalpy$).
...
For the Israel--Stewart viscous modes, the relaxation time $\taupi$
introduces an additional characteristic speed
\begin{equation}
\label{eq:rel-111-IS-speed}
v_{\mathrm{IS}} = \sqrt{\frac{\shearvisc}
  {\enthalpy\,\taupi}} \;\leq\; c,
\end{equation}
which must be enforced as a constraint on the constitutive
parameters.  This is the well-known result that the Israel--Stewart
theory is causal if and only if
$\taupi \geq \shearvisc/(\enthalpy c^{2})$.
...
(ii)~the Israel--Stewart parameters satisfy
$\taupi \geq \shearvisc/(\enthalpy c^{2})$.
...
propagate signals.  When combined with the Israel--Stewart framework
for viscous effects, all characteristic speeds remain bounded by $c$,
provided the constitutive parameters satisfy the constraints given
in \S\ref{sec:rel-111-causality}.
...
\item Israel--Stewart: $\taupi \geq \shearvisc/(\enthalpy c^{2})$.
