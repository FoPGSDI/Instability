...
%% =============================================================
%% Astrophysical application: Wave propagation in NS
%% =============================================================
\subsection*{Astrophysical application: magneto-inertial wave
  propagation in rotating neutron stars}

The dispersion relation~\eqref{eq:rel50-dispersion} governs the
propagation of magneto-inertial waves in the interiors of rotating,
magnetised neutron stars.  These waves couple the Coriolis and Lorentz
restoring forces, and their frequencies determine the observational
signatures of neutron star seismology
\citep{Israel2005,Strohmayer2005,Watts2006,AnderssonKokkotas1999}.

For a typical magnetar with spin period $P \sim 5$~s
($\Omega \sim 1.3$~rad\,s$^{-1}$), interior field
$B \sim 10^{15}$~G, and core density $\rho \sim 10^{14}$~g\,cm$^{-3}$,
the relativistic Alfv\'en speed is $\vA \sim 0.01$--$0.1\,c$.
The transition from rotation-dominated to field-dominated dynamics
occurs at the characteristic wavenumber
$k_{\mathrm{trans}} = 2\Omega/\vA
\sim 10^{-7}$--$10^{-6}$~cm$^{-1}$,
corresponding to wavelengths comparable to the stellar radius.

Figure~\ref{fig:rotmag-dispersion} shows the magneto-inertial
wave dispersion relation for several values of $\vA/c$.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.95\textwidth]{fig_rotmag_dispersion.pdf}
  \caption{Magneto-inertial wave dispersion in rotating neutron stars.
    \emph{Left:} Frequency $\omega$ versus wavenumber $k$ for the
    two transverse magneto-inertial branches at four values of
    $\vA/c$.  The light line $\omega = kc$ (dotted grey) is never
    crossed, confirming causality.
    \emph{Right:} Phase velocity $v_{\mathrm{ph}}/c$ versus $k$,
    showing that all modes remain sub-luminal.}
  \label{fig:rotmag-dispersion}
\end{figure}

The observational relevance includes:
\begin{itemize}
  \item \textbf{Magnetar QPOs.}  The discrete QPO frequencies observed
    in giant magnetar flares \citep{Israel2005,Watts2006} are
    interpreted as global Alfv\'en oscillations.  The relativistic
    dispersion relation shifts predicted frequencies by
    $\sim 1$--$10\%$.
  \item \textbf{Pulsar glitch recovery.}  Post-glitch relaxation
    involves magneto-inertial wave propagation through the superfluid
    core \citep{AnderssonKokkotas1999}.
  \item \textbf{Gravitational wave emission.}  Persistent
    magneto-inertial oscillations in rapidly rotating neutron stars
    can emit continuous gravitational waves at frequencies determined
