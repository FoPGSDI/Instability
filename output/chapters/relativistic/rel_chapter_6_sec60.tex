...
g_{\mathrm{eff}}(r) = \frac{e^{-\Lambda}}{(\varepsilon+p)}\,\frac{dp}{dr}
...
Israel--Stewart causal transport formalism (cf.\ \S\,\ref{sec:rel-1-2b}).
...
\mathscr{D}_{l,\mathrm{rel}}^{2}\,W_{\mathrm{rel}}
  = l(l+1)\frac{\psi}{\nu_{\mathrm{eff}}}\,G_{\mathrm{rel}},
\end{equation}
where $\nu_{\mathrm{eff}}$ is the effective kinematic viscosity
including the Israel--Stewart shear-relaxation correction
(see~\S\,\ref{sec:rel-1-2b}), and the relativistic angular operator is
\begin{equation}\label{eq:rel-60-11}
\mathscr{D}_{l,\mathrm{rel}}^{2} \equiv
  e^{-2\Lambda}\frac{d^{2}}{dr^{2}}
  +\frac{2}{r}\,e^{-2\Lambda}\frac{d}{dr}
  -\frac{l(l+1)}{r^{2}}
  - \frac{2}{r}\,\frac{d\Lambda}{dr}\,e^{-2\Lambda}\frac{d}{dr}.
\end{equation}
...
$\mathcal{C}$ of equation~\eqref{eq:6-133}.  The effective diffusivities
$\kappa_{\mathrm{eff}}$ and $\nu_{\mathrm{eff}}$ incorporate
Israel--Stewart relaxation:
\begin{equation}\label{eq:rel-60-17}
\kappa_{\mathrm{eff}} = \frac{\kappa}{1+\omega^{2}\tau_{q}^{2}},
\qquad
\nu_{\mathrm{eff}} = \frac{\nu}{1+\omega^{2}\tau_{\pi}^{2}},
\end{equation}
which at the marginal state ($\omega=0$) reduce to $\kappa$ and $\nu$,
respectively, recovering the classical values.
...
($\omega\neq 0$), the Israel--Stewart relaxation terms ensure that the
full time-dependent system is hyperbolic and all signal speeds remain
bounded by~$c$.  In particular, the thermal perturbation propagation
speed is $v_{\mathrm{th}} = \sqrt{\kappa/\tau_{q}}\leq c$.
\end{causalitycheck}
...
\end{equation}
The Israel--Stewart relaxation times for these processes are
$\tau_{q}\sim 10^{-18}$--$10^{-15}$~s (thermal) and
$\tau_{\pi}\sim 10^{-20}$--$10^{-17}$~s (shear), far shorter than
any hydrodynamic timescale.  Consequently, the Israel--Stewart
corrections to the effective diffusivities
(equation~\eqref{eq:rel-60-17}) are negligible for the crustal
convection problem, and the dominant relativistic effect is the
gravitational one encoded in $\mathscr{R}(\xi)$ and
$c_{\mathrm{rel}}(r)$.
...
purely conductive thermal transport.
