...
Newtonian viscous stress tensor by the Israel--Stewart causal
dissipative framework, thereby ensuring that the resulting variational
structure respects relativistic causality.
...
%----------------------------------------------------------------------
\subsection{Effect of viscosity: Israel--Stewart dissipation in
...
Newtonian viscous stress is replaced by the Israel--Stewart causal
dissipative tensor.
...
flux.  The Israel--Stewart evolution equations are
\begin{align}
  \taupi\,u^{\alpha}\nabla_{\alpha}\,\pi^{\langle\mu\nu\rangle}
  + \pi^{\mu\nu}
  &= 2\,\shearvisc\,\sigma^{\mu\nu},
  \label{eq:rel14-IS-shear}\\[4pt]
  \tau_{\Pi}\,u^{\alpha}\nabla_{\alpha}\,\Pi_{\mathrm{bulk}}
  + \Pi_{\mathrm{bulk}}
  &= -\bulkvisc\,\nabla_{\mu}\,u^{\mu},
  \label{eq:rel14-IS-bulk}
\end{align}
where $\sigma^{\mu\nu}$ is the shear tensor, $\shearvisc$ and
$\bulkvisc$ are the shear and bulk viscosity coefficients, and
$\taupi$, $\tau_{\Pi}>0$ are the relaxation times that guarantee
causal signal propagation.

Linearising the Israel--Stewart equations about the equilibrium and
...
For perturbations $\propto e^{i\sigma t}$, the Israel--Stewart
relation~\eqref{eq:rel14-IS-shear} gives at linear order
\begin{equation}
  \delta\pi^{\mu\nu}
  = \frac{2\,\shearvisc}{1 + i\sigma\,\taupi}\,
    \delta\sigma^{\mu\nu},
...
\begin{equation}
  ...
  + \frac{1}{2}\int_{\Sigma}
    \frac{\bulkvisc}{1+i\sigma\,\tau_{\Pi}}\,
...
In the \emph{Navier--Stokes limit} $\taupi\to 0$,
$\tau_{\Pi}\to 0$, one recovers $\Phi_{\mathrm{rel}}\to\Phi$ (the
classical dissipation integral~\eqref{eq:14-39}), and the dispersion
relation reduces to equation~\eqref{eq:14-41}.

For finite relaxation times the roots of
equation~\eqref{eq:rel14-dispersion-visc} are
\begin{equation}
  i\sigma
...
\end{equation}
Since both $\mathcal{I}>0$ and
$\operatorname{Re}\Phi_{\mathrm{rel}}>0$ (the latter guaranteed by
the second law of thermodynamics), the \emph{necessary and sufficient
condition for stability remains $\delta^{2}W>0$}, exactly as in the
non-dissipative case.

The key distinction from the Newtonian theory is that the relaxation
times $\taupi$ and $\tau_{\Pi}$ modify the damping rate and can
change the character of modes (oscillatory vs.\ over-damped) but
\emph{do not affect the stability boundary}.  Moreover, the finite
relaxation times ensure that all perturbation signals propagate at
speeds $\le c$, unlike the acausal instantaneous propagation implicit
in the Navier--Stokes viscous stress.
...
on $\mathcal{S}$.  The inclusion of Israel--Stewart causal dissipation
