...
\begin{equation}
  \rho_{\mathrm{eff}}
  = \frac{\enthalpy + \bsq\,c^{2}}{c^{2}}
  = \frac{\varepsilon + p}{c^{2}} + \frac{\bsq}{1}\,,
  \label{eq:rel-rhoeff}
\end{equation}
...
\paragraph{Heat equation (Israel--Stewart causal form).}
The classical diffusion equation $\nabla^{2}\Theta = -(\beta/\kappa)\,w$
is replaced by the Israel--Stewart causal heat equation:
\begin{equation}
  \tauq\,\frac{\partial^{2}\Theta}{\partial t^{2}}
  + \frac{\partial\Theta}{\partial t}
  = \kappa\,\nabla^{2}\Theta + \beta\,w\,,
  \label{eq:rel-heat-IS}
\end{equation}
where $\tauq$ is the heat-flux relaxation time.  For the marginal state
...
$k$.  The Israel--Stewart heat relaxation time $\tauq$ ensures that
thermal signals likewise propagate at finite speed
$v_{\mathrm{th}} = \sqrt{\kappa/\tauq} \leq c$.
\end{causalitycheck}
...
governed by $\vA^{2}/c^{2}$.  Writing $\vA^{2}/c^{2} = \mu_{0}H^{2}/
(4\pi\rho_{\mathrm{eff}}\,c^{2})$, we may estimate this ratio for
...
practical purposes in the laboratory.  Similarly, the Israel--Stewart
heat-relaxation time for mercury is $\tauq \sim 10^{-12}$~s, far below
any hydrodynamic time scale of interest; the causal heat equation
reduces to the classical Fourier law.
\end{relcorrection}
...
\begin{equation}
  ...
  = \frac{\mu_{0}\,B_{\mathrm{int}}^{2}\cos^{2}\!\vartheta\;d^{2}}
...
\noindent The Israel--Stewart causal dissipation theory that replaces
