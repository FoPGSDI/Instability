...
stratified fluid, in the Israel--Stewart framework with
vanishing relaxation times at marginal stability ($\sigma=0$), reduce
...
counterpart of this result within the Israel--Stewart causal
dissipative theory.
...
\subsection{Relativistic entropy production and the Israel--Stewart
second law}\label{sec:rel-14d}

The Israel--Stewart theory provides a natural framework for connecting
the variational principle to entropy production.  The entropy current
is
\begin{equation}
\label{eq:rel-entropy-current}
s^\mu = s\,n\,u^\mu + \frac{q^\mu}{T}
  - \frac{\tauq}{2\,\thermcond\,T^2}\,q_\alpha q^\alpha\,u^\mu
  + \ldots,
\end{equation}
where $s$ is the entropy per baryon and $n$ the baryon number density.
Its divergence is (Israel \& Stewart 1979)
\begin{equation}
...
= -\frac{\bulkP^2}{\bulkvisc\,T}
  ...
  + \text{(second-order relaxation terms)} \;\geqslant\; 0.
\end{equation}
The second law $\nabla_\mu s^\mu \geqslant 0$ is guaranteed
order-by-order provided the transport coefficients satisfy
$\shearvisc,\,\bulkvisc,\,\thermcond \geqslant 0$ and the relaxation
times $\taupi,\,\tauPi,\,\tauq > 0$.
...
\begin{enumerate}
\item The entropy current~\eqref{eq:rel-entropy-current} contains
  additional quadratic terms in the heat flux, required by the
  Israel--Stewart theory for causal propagation.  These ensure that
  the entropy production inequality~\eqref{eq:rel-entropy-production}
  is maintained even when heat signals propagate at finite speed.
...
The Israel--Stewart relaxation times ($\taupi$, $\tauq$, $\tauPi$)
do not appear explicitly in the marginal-stability equations
($\sigma = 0$), because all time derivatives vanish at the onset of
instability.  Causality is nevertheless preserved: the relaxation times
ensure that the \emph{growth} of perturbations (for $\Rarel$ slightly
above the critical value) propagates at finite speed.  The
characteristic speeds of the Israel--Stewart system are bounded by
\begin{equation}
v_{\mathrm{signal}}^2
= \frac{\shearvisc}{\enthalpy\,\taupi/c^2}
\leqslant c^2,
\end{equation}
which is guaranteed when the relaxation times satisfy the standard
causality bounds $\taupi \geqslant \shearvisc/(\enthalpy/c^2)\cdot c^{-2}$.
\end{causalitycheck}
...
\item The Israel--Stewart entropy production
  inequality~\eqref{eq:rel-entropy-production} reduces to the
  classical Clausius--Duhem inequality when all relaxation times
  vanish.
\end{enumerate}
