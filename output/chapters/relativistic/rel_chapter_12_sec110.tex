...
\section{Relativistic gravitational instability of a cylinder with an axial magnetic field}
\label{sec:rel-12-110}
...
(cf.\ \texttt{RELATIVISTIC\_CONVENTIONS.md})
\begin{equation}
...
the relativistic Alfv\'en speed is (cf.\ \texttt{RELATIVISTIC\_CONVENTIONS.md})
\begin{equation}
...
\paragraph{Quantitative estimates for individual AGN jets.}
The magnetic stabilization criterion provides quantitative predictions
for the knot spacing in observed relativistic
jets~\cite{Blandford2019}.  We apply the dispersion
relation~\eqref{eq:rel-12-110-17} to five well-studied AGN jets:

\begin{itemize}
\item \textbf{M87:} $R_{\mathrm{jet}}\sim 100\;\mathrm{pc}$,
  $\varepsilon\sim 10^{-9}\;\mathrm{erg/cm^{3}}$,
  $H_{z}\sim 10\;\mu\mathrm{G}$, $\Lf\sim 6$.  The critical field
  $H_{0,\mathrm{rel}} \sim 5\;\mu\mathrm{G}$, giving
  $H/H_{0,\mathrm{rel}}\sim 2$.  Predicted knot spacing
  $\lambda\sim 1\;\mathrm{kpc}$, observed $\sim 0.5$--$2\;\mathrm{kpc}$.

\item \textbf{3C~273:} $R_{\mathrm{jet}}\sim 500\;\mathrm{pc}$,
  $H_{z}\sim 5\;\mu\mathrm{G}$, $\Lf\sim 15$.  Predicted
  $\lambda\sim 10\;\mathrm{kpc}$, observed $\sim 5$--$20\;\mathrm{kpc}$.

\item \textbf{Cygnus~A:} $R_{\mathrm{jet}}\sim 1\;\mathrm{kpc}$,
  $H_{z}\sim 50\;\mu\mathrm{G}$, $\Lf\sim 3$.  Strong field
  ($H/H_{0,\mathrm{rel}}\gg 1$) exponentially suppresses fragmentation;
  consistent with the hotspot-dominated (smooth-jet) morphology.

\item \textbf{Centaurus~A:} $R_{\mathrm{jet}}\sim 50\;\mathrm{pc}$,
  $H_{z}\sim 20\;\mu\mathrm{G}$, $\Lf\sim 3$.  Predicted
  $\lambda\sim 0.5\;\mathrm{kpc}$, observed $\sim 0.3$--$0.7\;\mathrm{kpc}$.

\item \textbf{3C~31 (FR~I):} $R_{\mathrm{jet}}\sim 200\;\mathrm{pc}$,
  $H_{z}\sim 3\;\mu\mathrm{G}$, $\Lf\sim 2$.  Weak field;
  predicted $\lambda\sim 3\;\mathrm{kpc}$, observed $\sim 1.5$--$5\;\mathrm{kpc}$.
\end{itemize}

\noindent
Figure~\ref{fig:knot-spacing-data} compares the observed and predicted
knot spacings for these five jets, demonstrating that the magnetic
stabilization criterion provides order-of-magnitude agreement across
two decades in jet radius and magnetic field strength.
Figure~\ref{fig:magnetized-jet-knots} displays the
full dependence of the fragmentation scale and growth rate on the field
strength ratio.

\begin{figure}[ht]
\centering
\includegraphics[width=0.95\textwidth]{fig_magnetized_jet_knots.pdf}
\caption{%
\textbf{Left:} Dispersion relation for the gravitational instability of
a magnetized relativistic cylinder at different axial field strengths
$H/H_{0,\mathrm{rel}}$.  The magnetic field raises the critical
wavenumber (shorter minimum unstable wavelength) and reduces the peak
growth rate.
\textbf{Right:} Minimum unstable wavelength (solid, left axis) and
maximum growth rate (dashed, right axis) as functions of
$H/H_{0,\mathrm{rel}}$, with astrophysical jet regimes indicated.}
\label{fig:magnetized-jet-knots}
\end{figure}

\begin{figure}[ht]
\centering
\includegraphics[width=0.95\textwidth]{fig_knot_spacing_data.pdf}
\caption{%
\textbf{Left:} Observed versus predicted knot spacing for five AGN
jets (M87, 3C~273, Cygnus~A, Centaurus~A, 3C~31).  The predicted
spacing is computed from the magnetic-stabilization dispersion
relation~\eqref{eq:rel-12-110-17} using the relativistic gravitating
density $\rho_{G}$ and enthalpy $w_{\mathrm{tot}}$.  The dashed line
marks perfect agreement; the grey band a factor-of-three range.
\textbf{Right:} Theoretical knot spacing $\lambda_{\mathrm{knot}}/R_{\mathrm{jet}}$
versus magnetic field strength $H/H_{0,\mathrm{rel}}$, with
observed data points overlaid.  The Newtonian ($H=0$) fragmentation
scale $\lambda/R\approx 10.8$ is shown for reference.}
\label{fig:knot-spacing-data}
\end{figure}
...
%% ================================================================
\section*{BIBLIOGRAPHICAL NOTES (\S110)}
\label{sec:rel-12-110-bib}

The classical treatment of a magnetized self-gravitating cylinder is due
to:
\begin{enumerate}
...
\item S.~\textsc{Chandrasekhar}, \emph{Hydrodynamic and Hydromagnetic
  Stability}, Clarendon Press, Oxford, 1961, \S~110.
\end{enumerate}

\noindent
Relativistic MHD jet stability:
\begin{enumerate}
\setcounter{enumi}{2}
\item G.~\textsc{Bodo}, A.~\textsc{Mignone}, and
  R.~\textsc{Rosner}, `Linear stability analysis of magnetized
  relativistic jets: the non-rotating case', \emph{Mon.\ Not.\ Roy.\
  Astron.\ Soc.}\ \textbf{364}, 1165--82 (2005).

\item M.~\textsc{Hardee}, `Stability properties of strongly magnetized
  spine-sheath relativistic jets', \emph{Astrophys.\ J.}\ \textbf{664},
  26--46 (2007).

\item Y.~\textsc{Mizuno} et al., `Three-dimensional relativistic
  magnetohydrodynamic simulations of current-driven instability',
  \emph{Astrophys.\ J.}\ \textbf{700}, 684--93 (2009).
\end{enumerate}

\noindent
Knot morphology in observed jets:
\begin{enumerate}
\setcounter{enumi}{5}
\item E.~S.~\textsc{Perlman} et al., `The optical--near-infrared
  spectrum of the M87 jet from HST observations', \emph{Astrophys.\ J.\
  Lett.}\ \textbf{551}, L206--10 (2001).
...
\item L.~\textsc{Gavassino}, `Relativistic bulk viscous fluids of
  Burgers type and their presence in neutron stars',
  \emph{Phys.\ Rev.\ D}\ \textbf{107}, 096023 (2023);
  arXiv:2304.05455.

\item G.~\textsc{Camelio}, L.~\textsc{Gavassino},
  M.~\textsc{Antonelli}, S.~\textsc{Bernuzzi}, and
  B.~\textsc{Haskell}, `Simulating bulk viscosity in neutron stars.
  I.\ Formalism', \emph{Phys.\ Rev.\ D}\ \textbf{107}, 103031
  (2023); arXiv:2204.11809.

\item G.~\textsc{Camelio}, L.~\textsc{Gavassino},
  M.~\textsc{Antonelli}, S.~\textsc{Bernuzzi}, and
  B.~\textsc{Haskell}, `Simulating bulk viscosity in neutron stars.
  II.\ Evolution in spherical symmetry', \emph{Phys.\ Rev.\ D}\
  \textbf{107}, 103032 (2023); arXiv:2204.11810.
\end{enumerate}

%% ================================================================
\subsection*{Bulk viscosity and the stability of NS jet
  configurations}
\label{sec:rel-12-110-bulk}
%% ================================================================

The gravitational stability analysis of magnetised cylinders in this
section is directly applicable to the outer layers of differentially
rotating NS remnants, where the interplay between magnetic tension,
self-gravity, and bulk viscosity determines the morphology of any
magnetically collimated outflow.

In NS matter, bulk viscosity from Urca reactions provides a dissipative
mechanism that can damp compressible perturbations of the magnetised
cylinder.  The three-approach comparison of
\citet{CamelioGavassinoBulkI2022,CamelioGavassinoBulkII2022}
demonstrates that the Israel--Stewart bulk stress formalism faithfully
reproduces the full multi-component reacting-fluid dynamics for small
perturbations, but that when the muon fraction is dynamically important,
the Burgers-type model of \citet{GavassinoBurgers2023} is required.
In the Burgers model, the bulk stress obeys a second-order relaxation
equation with two characteristic timescales $\tau_{(1)}$ and
$\tau_{(2)}$ corresponding to the $e$-flavour and $\mu$-flavour Urca
channels, respectively.  The resulting frequency-dependent bulk viscosity
\begin{equation}
\label{eq:rel-12-zeta-Burgers}
\zeta_{\mathrm{eff}}^{\mathrm{Burgers}}(\omega) =
  \frac{\zeta_{(1)}}{1+\omega^{2}\tau_{(1)}^{2}}
  + \frac{\zeta_{(2)}}{1+\omega^{2}\tau_{(2)}^{2}}
\end{equation}
provides a richer dissipation spectrum than the single-relaxation IS
model, with implications for the damping of kink and sausage modes
in magnetised NS jet configurations.
