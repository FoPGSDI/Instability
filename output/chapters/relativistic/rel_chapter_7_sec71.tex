...
%  the Israel–Stewart causal dissipation framework.
%
...
\paragraph{Israel--Stewart causal corrections.}
The analysis above used the Eckart (first-order) dissipation
framework, in which the viscous stress is proportional to the
instantaneous strain rate.  In the Israel--Stewart second-order
theory (cf.\ \S\,\ref{sec:rel-framework-hydro}), the shear stress
satisfies a relaxation equation with relaxation time~$\taupi$:
\begin{equation}
\label{eq:rel-7-IS}
...
\end{equation}
In the marginal stationary state ($\sigma_{\mathrm{rel}} = 0$) the
time derivatives vanish and the Israel--Stewart equation reduces to
$\pi^{\mu\nu} = 2\shearvisc\,\sigma^{\mu\nu}$, which is the
Navier--Stokes relation.  Therefore the critical Taylor number for
\emph{stationary} onset is unaffected by the causal relaxation
terms.

For the \emph{oscillatory} marginal state (which will be examined
in the relativistic extension of~\S\,72), the relaxation
time~$\taupi$ will enter non-trivially and modify the conditions
for the onset of overstability.
