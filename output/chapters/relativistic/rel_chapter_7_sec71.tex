...
\citep{BemficaDN2018,Kovtun2019}, in which the anisotropic stress is
...
%% ================================================================
\subsection{Astrophysical application: Taylor--Couette laboratory
  analogue for narrow-gap relativistic flow}
\label{sec:rel-71-lab-analog}

The narrow-gap Taylor--Couette experiment provides the most direct
laboratory analogue of sheared astrophysical flows
\citep{Donnelly1991,JiGoodmanKageyama2001}.  While terrestrial
experiments operate at $\xi\ll 1$, the theoretical framework developed
here shows that the critical Taylor number scales universally as
\begin{equation}
\label{eq:rel-7-275}
T_{c}^{\mathrm{rel}} = (1+\xi)^{2}\,T_{c}^{\mathrm{cl}},
\end{equation}
with the critical wave number $a_{\min}\approx 3.12$ unchanged.

For astrophysical systems the correction is substantial.  In the
quark--gluon plasma ($\xi\approx 1/3$),
$T_{c}^{\mathrm{rel}}/T_{c}^{\mathrm{cl}}\approx 1.78$.  In the
ultra-relativistic limit ($p\sim\varepsilon$, $\xi\approx 1$), the
critical Taylor number quadruples.

Figure~\ref{fig:taylor-couette-narrow} presents two key results.  The
left panel shows the critical Taylor number as a function of $\mu$
for representative values of~$\xi$, demonstrating how the
relativistic enhancement affects the instability boundary across
the full range of angular-velocity ratios.  The right panel isolates
the ratio $T_{c}^{\mathrm{rel}}/T_{c}^{\mathrm{cl}}=(1+\xi)^{2}$
as a function of~$\xi$, with astrophysical regimes indicated.

These results suggest that proposed liquid-metal experiments designed
to probe the magnetorotational instability \citep{JiGoodmanKageyama2001}
could, with appropriate scaling of the Taylor number, also test the
enthalpy-inertia mechanism that governs relativistic centrifugal
instability in hot, dense astrophysical fluids.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\textwidth]{fig_taylor_couette_narrow_gap.pdf}
\caption{Left: critical Taylor number $T_{c}^{\mathrm{rel}}$ versus
  $\mu=\Omega_{2}/\Omega_{1}$ in the narrow-gap limit for several
  values of the relativistic correction parameter~$\xi$.
  Right: ratio $T_{c}^{\mathrm{rel}}/T_{c}^{\mathrm{cl}}=(1+\xi)^{2}$
  as a function of~$\xi$, with astrophysical regimes marked.}
\label{fig:taylor-couette-narrow}
\end{figure}

%--------------------------------------------------------------
\subsection{Systematic computation of the critical Taylor number
correction factor for five fluid systems}
\label{sec:rel-71-five-fluids}
%--------------------------------------------------------------

We now evaluate the correction factor $(1+\xi)^{2}$ quantitatively
for five representative fluid systems spanning the full range from
terrestrial laboratories to the early universe.  Recall from
equation~\eqref{eq:rel-7-218} that
$\xi = (\edensity + p - \rdensity c^{2})/(\rdensity c^{2})$, and
from~\eqref{eq:rel-7-219} that
$T_{c}^{\mathrm{rel}} = T_{c}^{\mathrm{cl}}\,(1+\xi)^{2}$.

\paragraph{(i) Water at $T = 293$\,K, $P = 1$\,atm.}
For liquid water the rest-mass energy density dominates overwhelmingly.
The specific internal energy is
$u \approx c_{v}\,T \approx 4.18\times 10^{3}\times 293
\approx 1.22\times 10^{6}\;\mathrm{J\,kg}^{-1}$, and the
pressure contribution is $p/\rdensity \approx 10^{5}/10^{3}
= 10^{2}\;\mathrm{J\,kg}^{-1}$.  Hence
\begin{equation}
\xi_{\mathrm{water}}
  = \frac{u + p/\rdensity}{c^{2}}
  \approx \frac{1.22\times10^{6}}{9\times10^{16}}
  \approx 1.36\times 10^{-11}.
\end{equation}
The correction factor is
$(1+\xi)^{2} \approx 1 + 2\xi \approx 1 + 2.7\times10^{-11}$,
entirely negligible.

\paragraph{(ii) Mercury at $T = 293$\,K.}
Mercury has $\rdensity = 13\,546\;\mathrm{kg\,m}^{-3}$,
$c_{v}\approx 139\;\mathrm{J\,kg}^{-1}\,\mathrm{K}^{-1}$,
$p\approx 10^{5}\;\mathrm{Pa}$.  The specific internal energy is
$u \approx 139\times 293 \approx 4.07\times10^{4}\;\mathrm{J\,kg}^{-1}$.
Thus
\begin{equation}
\xi_{\mathrm{Hg}}
  \approx \frac{4.07\times10^{4}}{9\times10^{16}}
  \approx 4.5\times10^{-13}.
\end{equation}
The correction $(1+\xi)^{2} \approx 1 + 9\times10^{-13}$ is
even more negligible than for water, as expected for a heavy
liquid metal with low specific heat.

\paragraph{(iii) Liquid sodium at $T = 500$\,K.}
Liquid sodium ($\rdensity \approx 880\;\mathrm{kg\,m}^{-3}$,
$c_{v}\approx 1.28\times10^{3}\;\mathrm{J\,kg}^{-1}\,\mathrm{K}^{-1}$)
is the working fluid in the Princeton MRI experiment.  At $T = 500$\,K,
$u \approx 1.28\times10^{3}\times500 = 6.4\times10^{5}\;\mathrm{J\,kg}^{-1}$,
so
\begin{equation}
\xi_{\mathrm{Na}}
  \approx \frac{6.4\times10^{5}}{9\times10^{16}}
  \approx 7.1\times10^{-12}.
\end{equation}
Again the correction is $\sim 10^{-11}$ and completely unmeasurable.

\paragraph{(iv) Neutron star crust matter at $\rdensity = 10^{14}\;\mathrm{g\,cm}^{-3}$.}
At nuclear density, the equation of state gives an internal energy
that is a non-trivial fraction of the rest-mass energy.  Using the
SLy equation of state~\citep{DouchinHaensel2001}, at
$\rdensity \approx 10^{14}\;\mathrm{g\,cm}^{-3}$ the energy density
is $\edensity \approx 1.05\,\rdensity c^{2}$ and the pressure
$p \approx 0.02\,\rdensity c^{2}$, so that
$(\edensity + p)/\rdensity c^{2} \approx 1.07$ and
\begin{equation}
\xi_{\mathrm{NS}}
  \approx 0.07.
\end{equation}
The correction factor is
$(1+\xi)^{2} \approx 1.145$, i.e.\ a $14.5\%$ enhancement of
the critical Taylor number---a significant relativistic correction
that would alter the onset of Taylor--Couette instability in any
differentially rotating neutron-star crust flow.

\paragraph{(v) Quark--gluon plasma at $T \sim 300$\,MeV.}
The QGP produced at RHIC and LHC can be approximated at leading
order as a conformal fluid with equation of state
$p = \edensity/3$.  However, the relevant ``rest-mass'' density is
the baryon number density times the nucleon mass:
$\rdensity c^{2} = n_{B}\,m_{N}\,c^{2}$.  At RHIC energies
($\sqrt{s_{NN}} = 200$\,GeV) near midrapidity, the net baryon
density is small compared with the entropy density, and the
ratio $(\edensity + p)/\rdensity c^{2}$ can be very large.
Taking the lattice QCD value $\edensity \approx 10\;\mathrm{GeV\,fm}^{-3}$
and $n_{B}\,m_{N}c^{2} \approx 1.5\;\mathrm{GeV\,fm}^{-3}$ gives
\begin{equation}
\frac{\edensity + p}{\rdensity c^{2}}
  \approx \frac{10 + 10/3}{1.5}
  \approx 8.9,
\qquad
\xi_{\mathrm{QGP}} \approx 7.9.
\end{equation}
The correction factor is
$(1+\xi)^{2} \approx (8.9)^{2} \approx 79$, meaning that the
critical Taylor number is enhanced by nearly two orders of magnitude.

\begin{table}[htbp]
\centering
\caption{Critical Taylor number correction factor $(1+\xi)^{2}$
for five fluid systems, evaluated using
$\xi = (\edensity + p)/(\rdensity c^{2}) - 1$.}
\label{tab:rel-five-fluids}
\begin{tabular}{lcccl}
\hline
Fluid system & $\xi$ & $(1+\xi)^{2}$
  & $T_{c}^{\mathrm{rel}}/T_{c}^{\mathrm{cl}}$
...
Mercury ($293$\,K) &
  ...
  $1 + 9\times10^{-13}$ &
  $\approx 1$ &
  thermal energy (low $c_{v}$) \\
Liquid sodium ($500$\,K) &
  $7.1\times10^{-12}$ &
  $1 + 1.4\times10^{-11}$ &
  $\approx 1$ &
  thermal internal energy \\
NS crust ($10^{14}\;\mathrm{g\,cm}^{-3}$) &
  $0.07$ &
  $1.145$ &
  $1.145$ &
  nuclear binding + pressure \\
QGP ($T\sim 300$\,MeV) &
  $7.9$ &
  $\approx 79$ &
  $\approx 79$ &
  thermal $\edensity \gg \rdensity c^{2}$ \\
\hline
\end{tabular}
\end{table}

\noindent
Several features of Table~\ref{tab:rel-five-fluids} deserve comment:
\begin{enumerate}
\item For all three terrestrial fluids (water, mercury, liquid sodium),
  the correction is $\lesssim 10^{-11}$ and has no observable
  consequence for Taylor--Couette experiments.  The hierarchy
  $\xi_{\mathrm{Hg}} < \xi_{\mathrm{Na}} < \xi_{\mathrm{water}}$
  reflects the ratio of specific heat per unit rest mass.

\item For neutron star crust matter, the $\sim 14.5\%$ correction is
  astrophysically significant: it modifies the angular velocity at
  which differential rotation in the NS crust becomes Taylor-unstable,
  potentially affecting post-merger dynamics and r-mode saturation
  amplitudes.

\item For the QGP, the correction is dramatic ($\sim$ two orders of
  magnitude), reflecting the ultra-relativistic nature of the fluid.
  This means that any Taylor--Couette-like instability in the QGP
  fireball at RHIC would have a vastly different critical angular
  velocity from the naive classical estimate.

\item The correction factor $(1+\xi)^{2}$ depends only on the
  thermodynamic state through $\xi$ and is independent of the
  viscosity, the gap width, and the angular velocity ratio $\mu$.
  This universality---a consequence of the spatial uniformity of
  the enthalpy-inertia factor in the narrow-gap limit---makes the
  comparison across systems particularly clean.
\end{enumerate}
