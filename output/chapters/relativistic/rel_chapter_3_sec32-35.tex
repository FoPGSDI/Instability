...
\subsection*{(c) Israel--Stewart corrections at $\Pran = 0$}

In the Israel--Stewart causal dissipation framework, the heat flux
...
The Navier--Stokes (first-order) theory would imply instantaneous thermal
propagation at $\Pran = 0$, violating causality.  The Israel--Stewart
formalism resolves this: even as $\Pran \to 0$, thermal signals propagate
at finite speed $v_{\mathrm{th}} = (\thermcond T / \tauq \enthalpy)^{1/2}
\leq c$.  The classical $\Pran = 0$ results of \S\,\ref{sec:3-32} are
...
higher-order terms are required by the Israel--Stewart formalism to
ensure the second law:
\begin{equation}\label{eq:rel-3-282}
  \covd s^{\mu} \geq 0.
\end{equation}

Computing the entropy production rate from the Israel--Stewart
constitutive relations, one obtains
...
\subsection*{(c) Oscillatory case: Entropy balance with Israel--Stewart terms}
...
time-varying kinetic energy and the Israel--Stewart relaxation
contributions.
...
Additionally, the Israel--Stewart relaxation terms contribute to the
...
\end{equation}
where $\dot{s}_{\mathrm{IS}}$ denotes the Israel--Stewart relaxation
...
\begin{enumerate}
  ...
  \item The Israel--Stewart relaxation time $\tauq$ enters through the
...
relativistic kinetic energy and the Israel--Stewart causal relaxation
effects.}
...
\begin{enumerate}
  \item \textbf{Quasi-periodic oscillations (QPOs):} Oscillations in
    ...
    \S\,\ref{sec:rel-3-32}\,(b).  The Israel--Stewart relaxation
...
Israel--Stewart causal dissipation theory, which underlies the
