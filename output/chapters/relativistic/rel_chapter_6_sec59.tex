...
%  Relativistic framework: TOV equilibrium + Israel-Stewart dissipation
...
corrections from both the enhanced inertia
$(\edensity + p)/c^{2}$ and the causal (Israel--Stewart) heat
transport.
...
(cf.\ the framework of the preceding relativistic sections), the
coupled eigenvalue equations become
\begin{equation}
\label{eq:rel-59-Weq}
\mathscr{D}_{l}^{2}\,W
...
\mathscr{D}_{l} \equiv
  ...
  - \frac{l(l+1)}{r^{2}},
\end{equation}
and the two relativistic parameters are
\begin{equation}
\label{eq:rel-59-params}
\Xi(r) \equiv \frac{p(r)}{\edensity(r)\,c^{2}},\qquad
\Upsilon \equiv \frac{\tauq\,\kappa_{T}}{R^{2}}.
\end{equation}
Here $\Xi(r)$ is the local pressure-to-energy ratio (spatially varying
through the TOV profile) and $\Upsilon$ is the dimensionless
Israel--Stewart relaxation parameter.  The relativistic
...
The Israel--Stewart relaxation time $\tauq > 0$ ensures that heat
propagates at a finite speed
$v_{\mathrm{heat}}^{2} = \kappa_{T}/(\tauq\,c_{v}) \leq c^{2}$.
The eigenvalue problem~\eqref{eq:rel-59-Weq}--\eqref{eq:rel-59-Feq}
reduces to the Newtonian equations of~\S\ref{sec:59} when
$\Xi\to 0$, $\Upsilon\to 0$, and $e^{-\Lambda}\to 1$.
All perturbation speeds remain subluminal for $\tauq > 0$.
\end{causalitycheck}
...
\frac{e^{-\Lambda}}{1+\Xi}
  ...
  + \order{\mathscr{C}^{2}},
\end{equation}
where $\Xi_{c} = p_{c}/(\edensity\,c^{2})$ is the central value of
...
Q_{jk}^{\mathrm{rel}}
  = \int_{0}^{R}\!
    W_{k}^{\mathrm{rel}}\,
    \frac{J_{l+\frac{1}{2}}(\alpha_{j}\,r)}{\sqrt{r}}\,
    \frac{e^{-\Lambda(r)}}{1+\Upsilon}\,
    dr.
\end{equation}
In the Newtonian limit ($\mathscr{C}\to 0$, $\Upsilon\to 0$) one
...
\mathcal{C}_{\mathrm{rel}}
  = \frac{\mathcal{C}}{
    (1+\bar{\Xi})(1+\Upsilon)\,\mathcal{G}(\mathscr{C})},
\end{equation}
where:
\begin{itemize}
...
\item $\Upsilon = \tauq\kappa_{T}/R^{2}$ is the Israel--Stewart
  parameter;
...
for a uniform-density sphere with $\Upsilon=0$ (instantaneous heat
...
\item[(ii)] \textbf{Metric curvature}
  ...
  the Newtonian case.
\item[(iii)] \textbf{Causal heat transport} ($\Upsilon > 0$): the
  finite relaxation time of the heat flux introduces a phase lag
  that modifies the critical threshold, typically (for moderate
  $\Upsilon$) providing an additional destabilising correction.
\end{enumerate}
...
$\mathscr{C}=0.10$ and $\Upsilon=0$, compared with the Newtonian
...
\bigskip
In the non-relativistic limit $\mathscr{C}\to 0$ and $\Upsilon\to 0$,
