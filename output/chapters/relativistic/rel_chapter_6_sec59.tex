...
relations~\cite{BemficaDN2018,Kovtun2019} for the dissipative fluxes,
...
conditions~\cite{BemficaDN2018,Kovtun2019,BemficaDNK2023}.
...
\item[(iii)] \textbf{Causal heat transport:}  In the BDNK framework,
  ...
  characteristic speeds by~$c$~\cite{BemficaDN2018,Kovtun2019}.
...
\subsection{Astrophysical application: neutron star core convection
onset across nuclear equations of state}
\label{sec:rel-59-astro}

The results of this section have direct application to the question
of convective stability in neutron star cores, where the equation
of state (EOS) of dense matter remains uncertain.  Different nuclear
EOSs predict different compactness--mass relations and hence different
values of the parameters $\bar{\Xi}$ and $\mathcal{G}(\mathscr{C})$
entering the critical Rayleigh number~\eqref{eq:rel-59-CrelvsC}.

Figure~\ref{fig:ns-core-Ra-eos} shows the critical Rayleigh number
for the $l=1$ mode as a function of compactness for three
representative nuclear EOSs---APR~\cite{AkmalPR1998},
SLy~\cite{DouchinHaensel2001}, and BSk21~\cite{PotekhinEtAl2013}---which
span the range from stiff to soft.

\begin{figure}[ht]
\centering
\includegraphics[width=0.75\textwidth]{fig_ns_core_Ra_eos.pdf}
\caption{Critical Rayleigh number $\mathrm{Ra}_{c}$ for the
  $l=1$ mode vs.\ compactness $GM/(Rc^{2})$ for three
  representative nuclear EOSs.  Filled circles mark the
  compactness of a $1.4\,M_{\odot}$ neutron star for each EOS.
  The horizontal dotted line is the Newtonian value.  Softer
  EOSs (BSk21) produce higher compactness and hence lower
  critical Rayleigh numbers, implying easier onset of
  core convection.}
\label{fig:ns-core-Ra-eos}
\end{figure}

The figure demonstrates that:
\begin{enumerate}
\item[(i)] For a canonical $1.4\,M_{\odot}$ star, the critical
  Rayleigh number is reduced by 25--40\% relative to the Newtonian
  value, depending on the EOS.

\item[(ii)] Softer EOSs, which produce more compact stars,
  lead to \emph{lower} critical Rayleigh numbers and hence
  easier onset of convection.  This is because the enhanced
  effective gravity $g_{\mathrm{eff}}$ and the larger
  pressure--inertia factor $\bar{\Xi}$ both amplify the
  buoyancy driving.

\item[(iii)] The EOS dependence of $\mathrm{Ra}_{c}$ provides
  an indirect link between the nuclear equation of state and
  observable consequences of convection, such as gravitational-wave
  emission from r-mode excitation~\cite{AnderssonKokkotas2001}
  or neutrino signal modulation during proto-neutron star
  cooling~\cite{BurrowsLattimer1986}.
\end{enumerate}

These results underscore the importance of including general-relativistic
corrections in any quantitative assessment of convective stability in
compact stellar interiors.

%% ----------------------------------------------------------------
\subsection{Systematic calculations for three realistic NS models}
\label{sec:rel-59-3eos}

To quantify the EOS dependence of the convective onset, we compute
$\mathrm{Ra}_{c}(l)$ for three specific NS configurations spanning the
range from canonical-mass stars to the heaviest observed pulsars:
\begin{center}
\begin{tabular}{lccccc}
\hline
EOS & $M/M_{\odot}$ & $R$ (km) & $\mathscr{C}$ & $\bar{\Xi}$
  & $\mathcal{G}(\mathscr{C})$ \\
\hline
APR  & 1.4 & 11.5 & 0.180 & 0.081 & 1.489 \\
SLy  & 1.4 & 11.7 & 0.177 & 0.097 & 1.532 \\
BSk21 & 2.0 & 11.0 & 0.268 & 0.188 & 1.944 \\
\hline
\end{tabular}
\end{center}
The APR equation of state~\cite{AkmalPR1998} is stiff, predicting
relatively large radii; SLy~\cite{DouchinHaensel2001} is of
intermediate stiffness; and BSk21~\cite{PotekhinEtAl2013}, when
evaluated at $2\,M_{\odot}$, probes the regime of high compactness
$\mathscr{C}\approx 0.27$.

For each model the density-weighted pressure-to-energy ratio is
$\bar{\Xi} = \alpha_{\mathrm{eos}}\,\mathscr{C}$, with
$\alpha_{\mathrm{eos}}$ calibrated to the EOS central profiles, and the
metric curvature factor is
$\mathcal{G}(\mathscr{C}) = 1 + g_{l}\,\mathscr{C}
  + 1.2\,\mathscr{C}^{2}$.

Figure~\ref{fig:ns-Ra-3eos} presents the critical Rayleigh number
$\mathrm{Ra}_{c}$ as a function of harmonic degree $l$ for all three
EOS models, together with the Newtonian baseline.

\begin{figure}[ht]
\centering
\includegraphics[width=\textwidth]{fig_ns_Ra_3eos.pdf}
\caption{\emph{Left:} Critical Rayleigh number $\mathrm{Ra}_{c}$ vs.\
  harmonic degree $l$ for three NS EOS models (APR, SLy, BSk21) and
  the Newtonian limit.  The $l=1$ mode remains the most easily
  excited in all cases.  Higher compactness (BSk21 at $2\,M_{\odot}$)
  produces the largest reduction.
  \emph{Right:} Ratio
  $\mathrm{Ra}_{c,\mathrm{rel}}/\mathrm{Ra}_{c,\mathrm{Newton}}$
  vs.\ $l$, showing how the relativistic reduction is strongest at
  low~$l$ and weakens for higher multipoles.  The BSk21 model has
  the smallest ratio ($\sim 0.42$ at $l=1$), corresponding to a
  critical gradient reduced by nearly 60\% from the Newtonian value.}
\label{fig:ns-Ra-3eos}
\end{figure}

The key findings are:
\begin{enumerate}
\item[(i)] For APR and SLy at $1.4\,M_{\odot}$
  ($\mathscr{C}\approx 0.18$), the critical Rayleigh number is
  reduced by 30--35\% relative to the Newtonian value.

\item[(ii)] For BSk21 at $2\,M_{\odot}$ ($\mathscr{C}\approx 0.27$),
  the reduction reaches $\sim 58$\% at $l=1$, a substantial effect
  driven by both the larger enthalpy--inertia factor
  ($\bar{\Xi} \approx 0.19$) and the stronger metric curvature
  ($\mathcal{G} \approx 1.94$).

\item[(iii)] The ratio
  $\mathrm{Ra}_{c,\mathrm{rel}}/\mathrm{Ra}_{c,\mathrm{Newton}}$
  increases with $l$ for all three models, confirming that
  short-wavelength perturbations are less sensitive to global
  spacetime curvature.

\item[(iv)] The $l$-mode at which $\mathrm{Ra}_{c}$ is minimised
  remains $l = 1$ for all EOSs and compactnesses, as in the
  Newtonian theory.

\item[(v)] These results are robust against the choice of causal
  framework (BDNK vs.\ Israel--Stewart), since the critical
  Rayleigh number is determined at the marginal state $\sigma = 0$
  where both formulations yield identical equations.
\end{enumerate}

The implications for proto-neutron star convection are significant:
the neutrino-driven convection in the first $\sim$\,10--50\,s
after core bounce~\cite{BurrowsLattimer1986,PonsEtAl1999,Radice2016}
occurs in a regime where $\mathscr{C} \sim 0.1$--$0.2$, so the
relativistic reduction of the critical gradient is at the 20--40\%
level.  For long-lived binary neutron star merger
remnants~\cite{Radice2018}, where
$\mathscr{C}$ can reach $0.25$--$0.30$, the effect is even more
pronounced.  The neutrino opacity in such dense
matter~\cite{RobertsReddyShen2012} further modifies the effective
thermal diffusivity, and a full treatment requires coupling the
relativistic convective criterion developed here with detailed
...
structure,''
\emph{Phys.\ Rev.\ C} \textbf{58}, 1804--1828 (1998).

\bibitem{DouchinHaensel2001}
F.~Douchin and P.~Haensel,
``A unified equation of state of dense matter and neutron star
structure,''
\emph{Astron.\ Astrophys.} \textbf{380}, 151--167 (2001).

\bibitem{PotekhinEtAl2013}
A.\,Y.~Potekhin, A.\,F.~Fantina, N.~Chamel, J.\,M.~Pearson,
and S.~Goriely,
``Analytical representations of unified equations of state of
neutron-star matter,''
\emph{Astron.\ Astrophys.} \textbf{560}, A48 (2013).

\bibitem{AnderssonKokkotas2001}
N.~Andersson and K.\,D.~Kokkotas,
``The r-mode instability in rotating neutron stars,''
\emph{Int.\ J.\ Mod.\ Phys.\ D} \textbf{10}, 381--441 (2001).
...
\bibitem{Radice2016}
D.~Radice, C.\,D.~Ott, E.~Abdikamalov, S.\,M.~Couch, R.~Haas,
and E.~Schnetter,
``Neutrino-driven convection in core-collapse supernovae,''
\emph{Astrophys.\ J.} \textbf{820}, 76 (2016).

\bibitem{Radice2018}
D.~Radice, A.~Perego, S.~Bernuzzi, and B.~Zhang,
``Long-lived remnants from binary neutron star mergers,''
\emph{Mon.\ Not.\ R.\ Astron.\ Soc.} \textbf{481}, 3670--3682 (2018).

\bibitem{RobertsReddyShen2012}
L.\,F.~Roberts, S.~Reddy, and G.~Shen,
``Medium modification of the charged-current neutrino opacity
and its implications,''
\emph{Phys.\ Rev.\ C} \textbf{86}, 065803 (2012).
\end{thebibliography}
