...
\end{equation}
where $\enthalpy = (\edensity + p)/c^{2}$ is the relativistic enthalpy density,
...
\end{equation}
where the factor $\enthalpy/c^2$ multiplying the inertial and
Coriolis terms replaces the Newtonian $\rho_0$.  In the BDNK
first-order framework, the dissipative contributions (shear viscosity
and heat flux) enter algebraically through the constitutive relations;
no auxiliary relaxation terms appear in the momentum equation.

The heat equation, in the BDNK causal first-order framework, retains
the standard diffusion form at the hydrodynamic level:
\begin{equation}\label{eq:rel-3-79}
  \frac{\partial\Theta}{\partial t}
  = \beta\,w + \kappa\,\nabla^2\Theta,
\end{equation}
where causality is ensured by the BDNK frame coefficients (which
modify the characteristic speeds of the coupled system) rather than by
a relaxation time.  This is the classical equation~\eqref{eq:3-79}
with the relativistic corrections to $\kappa$ and $\beta$ absorbed
...
In the BDNK first-order theory, the heat equation~\eqref{eq:rel-3-79}
is rendered causal by the frame-dependent transport coefficients, which
ensure that all characteristic speeds of the coupled system satisfy
...
% =====================================================================
% DEEP RESEARCH 2: Systematic realistic MSP calculation
% =====================================================================
\section{Realistic millisecond pulsar model: SLy EOS and PSR~J1748$-$2446ad}
\label{sec:rel-3-deep-msp}

We now present a systematic calculation of the critical Rayleigh number
for the fastest known millisecond pulsar, PSR~J1748$-$2446ad
\citep{HesselsEtAl2006}, using nuclear equation-of-state data
and the relativistic formalism developed above.
...
\hline
Mass $M$ & $1.4\,M_\odot$ & canonical \\
Radius $R$ & $12$~km & NICER constraints \citep{RileyEtAl2021} \\
Spin frequency $f_{\rm spin}$ & $716$~Hz & \citet{HesselsEtAl2006} \\
Angular velocity $\Omega$ & $4498$~rad\,s$^{-1}$ & $2\pi f_{\rm spin}$ \\
Core density $\rho$ & $\sim 3\rho_{\rm nuc}$ & $\rho_{\rm nuc} = 2.8\times 10^{14}$~g\,cm$^{-3}$ \\
Surface gravity $g$ & $1.3\times 10^{14}$~cm\,s$^{-2}$ & $GM/R^2$ \\
\hline
\end{tabular}
\end{center}

For the SLy equation of state \citep{DouchinHaensel2001}, the
enthalpy ratio at $\rho \approx 3\rho_{\rm nuc}$ is
\begin{equation}\label{eq:xi-SLy}
  \xi_{\rm SLy}
  \equiv \frac{\enthalpy}{\rdensity c^2}
  = \frac{\edensity + p}{\rdensity c^2}
  \approx 1.28.
\end{equation}
This accounts for the nuclear binding energy and pressure contribution
to the relativistic enthalpy.  For comparison, the APR equation of state
\citep{AkmalPR1998} gives $\xi_{\rm APR} \approx 1.15$ (softer EOS),
while stiffer equations of state can yield $\xi \approx 1.45$.

\subsection{Transport coefficients}

The viscosity of neutron star matter has been recently re-evaluated
by \citet{CamelioEtAl2023}, who computed the shear and bulk
viscosities including superfluid effects and their impact on r-mode
damping.  The bulk viscosity receives important relativistic
corrections as shown by \citet{Gavassino2024}.  We adopt:
\begin{center}
\begin{tabular}{lll}
\hline
Quantity & Value & Reference \\
\hline
Kinematic viscosity $\nu$ & $1$--$100$~cm$^2$\,s$^{-1}$ & \citet{CamelioEtAl2023} \\
Thermal diffusivity $\kappa$ & $\sim 10^5$~cm$^2$\,s$^{-1}$ & electron conduction \\
Prandtl number Pr & $10^{-5}$--$10^{-3}$ & $\nu/\kappa$ \\
Thermal expansion $\alpha$ & $\sim 10^{-4}$~K$^{-1}$ & degenerate nuclear matter \\
\hline
\end{tabular}
\end{center}

\subsection{Computation of $\Tarel$ and $\Rarel^{(c)}$}

The relativistic effective kinematic viscosity is
\begin{equation}
  \nu_{\rm eff} = \frac{\nu}{\xi_{\rm SLy}} = \frac{\nu}{1.28},
\end{equation}
giving the relativistic Taylor number
\begin{equation}
  \Tarel = \frac{4\Omega^2 d^4}{\nu_{\rm eff}^2}
         = \frac{4\Omega^2 d^4 \xi^2}{\nu^2}
         = \xi^2\,\mathscr{T}.
\end{equation}

For $\nu = 10$~cm$^2$\,s$^{-1}$ and $d = 100$~m:
\begin{align}
  \mathscr{T}_{\rm Newt} &= \frac{4 \times (4498)^2 \times (10^4)^4}{10^2}
    = 8.1 \times 10^{21}, \\
  \Tarel &= (1.28)^2 \times 8.1 \times 10^{21}
    = 1.33 \times 10^{22}.
\end{align}

The critical Rayleigh number from the exact solution of the cubic
$2x^3 + 3x^2 = 1 + \Tarel/\pi^4$ (equation~\eqref{eq:rel-3-131}) gives:
\begin{align}
  \Rarel^{(c)}_{\rm Newt} &= 3.51 \times 10^{15}, \\
  \Rarel^{(c)}_{\rm rel} &= 4.87 \times 10^{15}.
\end{align}

The relativistic correction is therefore:
\begin{equation}\label{eq:msp-correction}
  \frac{\Delta\Rarel^{(c)}}{\Rarel^{(c)}}
  = \frac{\Rarel^{(c)}_{\rm rel} - \Rarel^{(c)}_{\rm Newt}}
         {\Rarel^{(c)}_{\rm Newt}}
  \approx 39\%.
\end{equation}

\begin{relcorrection}
For realistic millisecond pulsar parameters with the SLy EOS
($\xi = 1.28$), the relativistic correction to the critical Rayleigh
number is $\sim 39\%$---far from negligible.  This correction grows
as $\xi^{4/3}$ in the asymptotic ($\Tarel \gg 1$) regime, following
equation~\eqref{eq:rel-3-134}.  For the stiffest allowed equations
of state ($\xi \approx 1.45$), the correction reaches $\sim 70\%$.
\end{relcorrection}

Figure~\ref{fig:msp-realistic-Ra} displays the critical Rayleigh
number as a function of the physical Taylor number for several EOS
models, with the PSR~J1748$-$2446ad parameter range highlighted.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.95\textwidth]{fig_msp_critical_Ra_realistic.pdf}
  \caption{Critical Rayleigh number $\mathrm{Ra}_c(\mathrm{Ta})$ for
    rotating convection with realistic millisecond pulsar parameters.
    Curves are shown for Newtonian ($\xi = 1$) and three nuclear EOS
    models: APR ($\xi = 1.15$), SLy ($\xi = 1.28$), and a stiff EOS
    ($\xi = 1.45$).  Markers indicate specific transport coefficient
    values for PSR~J1748$-$2446ad ($f_{\rm spin} = 716$~Hz,
    $d = 100$~m); circles: $\nu = 1$, squares: $\nu = 10$, diamonds:
    $\nu = 100$~cm$^2$\,s$^{-1}$.  The shaded band spans the
    plausible MSP parameter range.  The relativistic correction is
    $\sim 39\%$ for SLy at the reference parameters.}
  \label{fig:msp-realistic-Ra}
\end{figure}
...
\item F.~\textsc{Douchin} and P.~\textsc{Haensel}, `A unified equation
  of state of dense matter and neutron star structure',
  \emph{Astron.\ Astrophys.}, \textbf{380}, 151--67 (2001).

\item T.~E.~\textsc{Riley} and others, `A NICER view of the massive
  pulsar PSR~J0740+6620', \emph{Astrophys.\ J.\ Lett.}, \textbf{918},
  L27 (2021).
\end{enumerate}
