...
and the causal (Israel--Stewart) heat-flux relaxation time~$\tauq$;
...
perturbation equations in the Boussinesq--Eckart limit
...
\begin{equation}\label{eq:rel-Ra-def}
...
$\enthalpy/\rdensity c^{2}\to 0$ and $\tauq\to 0$),
equations~\eqref{eq:rel-2-277}--\eqref{eq:rel-2-278} reduce to the
classical pair~(277)--(278) of \S\,\ref{sec:17}.

\begin{causalitycheck}
The relaxation term proportional to~$\tauq$ in
equation~\eqref{eq:rel-2-278} renders the heat equation hyperbolic.
Perturbations therefore propagate at a finite speed bounded by
\[
  v_{\mathrm{heat}} = \sqrt{\frac{\kappa_{T}}{\tauq}} \leq c,
\]
as required by the Israel--Stewart theory.  Setting $\tauq = 0$ recovers
the parabolic (acausal) Fourier law.
\end{causalitycheck}
...
\begin{equation}\label{eq:rel-2-279}
  ...
  \tauq\,\frac{\partial F}{\partial t}\bigg|_{z=\pm 1/2} = 0,
\end{equation}
the last condition ensuring that the causal heat flux vanishes at the
walls.  For the marginal state ($\partial/\partial t = 0$, corresponding
...
$\Rarel/\Ra \sim 5/3$.  The causal relaxation times $\tauq$ and
$\taupi$ are of order $1/T \sim 1$\,fm/$c$, which is comparable to
the hydrodynamic evolution time of the fireball; thus the full
Israel--Stewart dynamics (not merely the Navier--Stokes limit) governs
...
Israel--Stewart equations in a B\'enard-like configuration has been
...
\item \textit{Causal vs.\ acausal heat transport.}  Replacing the
  Israel--Stewart heat equation by the acausal Fourier law
  ($\tauq = 0$) produces spurious, superluminal thermal
  runaway in the nonlinear regime, even though the linear onset
  threshold is unaffected (the marginal mode is stationary).  This
  underscores the necessity of the causal formulation for any
  finite-amplitude analysis.
...
\emph{must} employ a causal heat-transport law (Israel--Stewart or an
equivalent hyperbolic formulation).  The Eckart or Fourier
...
once the dissipation function is computed from the Israel--Stewart
...
hyperbolic relaxation equations.  It is the minimal causal extension of
relativistic dissipative fluid dynamics and provides the framework for
...
hydrodynamics.  Although superseded by the Israel--Stewart theory for
