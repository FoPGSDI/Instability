...
Israel--Stewart causal dissipation formalism and covariant MHD
developed in the relativistic framework chapters.
...
of thermal expansion, and we have used the Israel--Stewart shear
...
The linearized \emph{energy equation} with Israel--Stewart causal heat
conduction replaces the classical equation~(23):
\begin{equation}
...
\end{equation}
where $\tauq$ is the heat-flux relaxation time from the Israel--Stewart
theory.  The term $\tauq\,\partial^{2}\Theta/\partial t^{2}$ converts
the parabolic heat equation into a \emph{hyperbolic} (telegraph)
...
\begin{equation}
  \bigl(D^{2}-a^{2}-p_{1}\sigma
    - \tauq\,p_{1}\,\sigma^{2}/d^{2}\bigr)\,\Theta
...
replacements $R\to\Rarel$, $T\to\Tarel$, $Q\to\Qrel$, and the addition
of the Israel--Stewart term $\tauq\,p_{1}\sigma^{2}/d^{2}$ in the
thermal operator.

\begin{causalitycheck}
\textbf{Thermal modes:} The Israel--Stewart term converts the heat
operator from $(D^{2}-a^{2}-p_{1}\sigma)$ to
$(D^{2}-a^{2}-p_{1}\sigma - \tauq p_{1}\sigma^{2}/d^{2})$, which is a
second-order polynomial in~$\sigma$.  The thermal signal speed is
$v_{\mathrm{th}} = \sqrt{\kappa/\tauq}$, and the condition
$v_{\mathrm{th}} \le c$ is satisfied provided
$\tauq \ge \kappa/c^{2}$.
...
Note that the Israel--Stewart relaxation term drops out entirely at
$\sigma=0$: the marginal-stability criterion for stationary convection
is \emph{insensitive} to the causal modification of heat transport.
This is exactly as expected---the relaxation time affects only the
...
\item The effective inertia $\enthalpy^{*}_{0}$ replaces $\rho_{0}$,
  ...
  values.
\item The Israel--Stewart heat relaxation does not affect the
...
$B_{0}^{2}\ll 4\pi\rho_{0}c^{2}$, and $\tauq\to 0$:
\begin{align}
...
\eqref{eq:rel51-master} (with $\tauq\to 0$) reduces to
equation~(43).\qed
...
\S\S\,51--52:}
\begin{enumerate}
\item \textbf{Thermal diffusion:} Governed by the telegraph equation
  with signal speed $v_{\mathrm{th}}=\sqrt{\kappa/\tauq}\le c$
  (by requiring $\tauq\ge\kappa/c^{2}$).
...
\item \textbf{Viscous diffusion:} Israel--Stewart shear relaxation
  bounds the viscous signal speed at
  $v_{\mathrm{visc}}=\sqrt{\shearvisc/(\enthalpy^{*}_{0}\taupi)}
  \le c$ (requiring $\taupi\ge\shearvisc/(\enthalpy^{*}_{0}c^{2})$).
\end{enumerate}
