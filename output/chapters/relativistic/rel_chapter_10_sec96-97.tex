...
%% ================================================================
\section{Astrophysical application: Magnetic stabilisation of RT
instability in relativistic jets}\label{sec:rel-96-97-astro}

Relativistic jets from active galactic nuclei (AGN) and gamma-ray
bursts (GRBs) provide a compelling astrophysical setting for the
magnetically stabilised RT instability \citep{Matsumoto2013RT,Mignone2010}.
As the jet propagates through the ambient medium, it decelerates and
drives a cocoon of shocked material.  The jet--cocoon interface is
RT-unstable under this deceleration; however, the ordered magnetic
field (predominantly toroidal in the jet frame) provides a stabilising
tension that can suppress the growth of RT fingers.

The critical field strength for stabilisation is determined by
equation~\eqref{eq:rel97-kcrit}: modes with wavevector component
$k_{x} > k_{x,\mathrm{crit}}$ along the field direction are
magnetically stabilised.  In the relativistic regime, the effective
stabilisation is \emph{enhanced} by the factor $1/(1-v_{A}^{2}/c^{2})$
in the equivalent surface tension~\eqref{eq:rel97-Teq}, reflecting
the additional inertia of the magnetic field energy.  For jets with
magnetisation parameter $\sigma_{B} = b^{2}/(\rho c^{2}) \sim 0.1$--$1$,
the Alfv\'en speed reaches $v_{A}/c \sim 0.3$--$0.7$, and the
relativistic enhancement of the magnetic stabilisation becomes
significant \citep{Bromberg2019,GourgouliatosKomissarov2018}.

The interchange instability (equation~\eqref{eq:rel97-interchange})
remains the dominant channel for disruption: perturbations
perpendicular to the field are completely unaffected by magnetic
tension, regardless of field strength \citep{Matsumoto2017}.

Figure~\ref{fig:magnetic-RT-jet} illustrates the magnetic stabilisation
of the RT instability in relativistic jets.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\textwidth]{fig_magnetic_RT_jet.pdf}
\caption{Magnetic stabilisation of RT instability in relativistic jets.
  \textit{Left:} Critical magnetic field strength for complete
  stabilisation vs.\ enthalpy ratio, for different Alfv\'en speeds.
  The relativistic correction factor $\sqrt{1 - v_{A}^{2}/c^{2}}$
  reduces the threshold.  \textit{Right:} Growth rate vs.\
  wavenumber for a vertical magnetic field with $v_{A}/c = 0.5$.
  The growth rate saturates at $k \to \infty$ (dotted lines),
  unlike the unbounded $\sqrt{gk}$ behaviour of the unmagnetised
  case (dashed).}
\label{fig:magnetic-RT-jet}
\end{figure}

%% ================================================================
\subsection{Deep calculation: Critical magnetic field vs jet Lorentz factor}
\label{sec:rel-96-97-deep-jet}

We now present a detailed calculation of the critical magnetic field
required for RT stabilisation at the jet--cocoon interface as a function
of the jet Lorentz factor~$\Gamma_{\mathrm{jet}}$.

\paragraph{Physical model.}
A relativistic jet with bulk Lorentz factor $\Gamma_{\mathrm{jet}}$
propagates through an ambient medium and drives a cocoon of shocked
material.  In the lab frame, the jet enthalpy density is boosted by
$\Gamma^{2}$: $w_{\mathrm{jet,lab}} \sim \Gamma^{2}\,w_{\mathrm{jet,com}}$,
while the cocoon remains approximately at rest.  The resulting
relativistic Atwood number increases with~$\Gamma$, making
higher-$\Gamma$ jets more susceptible to RT disruption
\citep{Duffell2011,Matsumoto2021}.

\paragraph{Magnetisation threshold.}
From the horizontal-field dispersion relation~\eqref{eq:rel97-dispersion},
the critical magnetisation parameter
$\sigma_{B} = b^{2}/(\rho c^{2})$ for complete RT stabilisation satisfies
\begin{equation}
  \mathcal{A}_{\mathrm{rel}}
  = \frac{2\sigma_{B}(1+\sigma_{B})}{1+2\sigma_{B}},
\end{equation}
which we solve for $\sigma_{B,\mathrm{crit}}$ as a function
of~$\Gamma_{\mathrm{jet}}$ and the density ratio
$\rho_{\mathrm{coc}}/\rho_{\mathrm{jet}}$.  For high-$\Gamma$
jets ($\Gamma \gtrsim 10$), the critical magnetisation saturates at
$\sigma_{B,\mathrm{crit}} \sim 0.5$, corresponding to
$v_{A}/c \sim 0.6$.

\paragraph{AGN vs GRB jets.}
For AGN jets ($\Gamma \sim 2$--$10$), modest magnetisations
$\sigma_{B} \sim 0.01$--$0.1$ suffice for partial stabilisation.
For GRB jets ($\Gamma \sim 100$--$300$), the critical
magnetisation is significantly higher, and the interchange
instability (perpendicular to the field) remains completely
unsuppressed regardless of field strength.

Figure~\ref{fig:jet-RT-deep} presents the critical magnetisation
and Alfv\'en speed as functions of jet Lorentz factor, along with
the growth rate dispersion for both horizontal and vertical field
configurations.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\textwidth]{fig_jet_rt_magnetic.pdf}
\caption{Deep calculation: Magnetic RT stabilisation in
  relativistic jets.
  \textit{(a)}~Critical magnetisation $\sigma_{B,\mathrm{crit}}$
  vs.\ jet Lorentz factor for different density ratios; the AGN
  and GRB regimes are shaded.
  \textit{(b)}~Critical Alfv\'en speed $v_{A}/c$ at the
  stabilisation threshold, showing that causality ($v_{A}<c$) is
  never violated.
  \textit{(c)}~Growth rate vs.\ wavenumber for a horizontal field
  at $\Gamma = 10$ with different magnetisations.
  \textit{(d)}~Vertical-field growth rate saturation for different
  $\Gamma$ at fixed $\sigma_{B}=0.3$.}
\label{fig:jet-RT-deep}
\end{figure}
