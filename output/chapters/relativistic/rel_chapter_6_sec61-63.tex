...
\item \textbf{Causal dissipation.}  Fourier heat conduction is
  replaced by the Israel--Stewart equation (see
  RELATIVISTIC\_CONVENTIONS.md), ensuring that thermal perturbations
  propagate at a finite speed bounded by~$c$.
...
where:
\begin{itemize}
\item $\shearvisc$ is the shear viscosity (Israel--Stewart);
...
The linearised heat equation, using Israel--Stewart causal transport,
takes the form
\begin{equation}\label{eq:rel-6-218}
  ...
  = \beta\,r\,\delta v_{r}
    + \kappa_{T}\,\hat{\nabla}^{2}\Theta,
\end{equation}
where $\tauq$ is the heat-flux relaxation time.  Setting
$\tauq = 0$ recovers the classical parabolic equation~(6-218).
...
\begin{equation}\label{eq:rel-6-242}
  ...
  \qquad\text{at } r = 1,
\end{equation}
as in equation~(6-242).  The Israel--Stewart formulation does not
alter the boundary conditions on the velocity field; however, the
relaxation equation for the heat flux requires an additional
condition:
\begin{equation}\label{eq:rel-6-242b}
  \tauq\,\frac{\partial\Theta}{\partial t}
  + \Theta = 0
  \qquad\text{at } r = 1,
\end{equation}
which reduces to $\Theta = 0$ in the stationary ($p = 0$) case.
...
\item \textbf{Neutrino transport.}  The effective thermal
  ...
  In the diffusive regime (interior of the PNS), the Israel--Stewart
  relaxation time $\tauq$ is of order the neutrino mean free time,
  $\tauq \sim \lambda_{\nu}/c \sim 10^{-7}$\,s.  Causal transport
  corrections are thus small compared to the convective overturn
  time ($\sim$\,1\,ms) but must be retained for formal consistency.
...
\noindent Relativistic fluid dynamics and causal dissipation:
