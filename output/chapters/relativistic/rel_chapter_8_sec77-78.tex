...
relativistic fluid governed by Israel--Stewart causal dissipation.
...
\item[(ii)] The Israel--Stewart shear relaxation equation introduces a
  finite relaxation time $\taupi$ for the viscous stress tensor.
  In the linearised perturbation equations, this manifests as a
  replacement of the diffusion operator $(D^2 - a^2 - \sigma)$ by
  $(D^2 - a^2 - \sigma - \taupi\,\sigma^2\,\nu^{-1}d^{-2})$ in the
  viscous terms.
\end{enumerate}

We define the Israel--Stewart correction parameter
\begin{equation}
\label{eq:rel-8-Psi}
\Psi \;\equiv\; \frac{\taupi\,\nu}{d^2},
\end{equation}
which measures the ratio of the viscous relaxation time to the
diffusion time across the gap.
...
\item The \emph{Israel--Stewart relaxation} modifies the viscous
...
Israel--Stewart relaxation term $\Psi\sigma^2$ vanishes identically,
...
$10^{-10}$, and the Israel--Stewart relaxation parameter $\Psi$ is
...
\item \textbf{Viscous signal propagation (Israel--Stewart).}
  In the Navier--Stokes theory, viscous signals propagate
  instantaneously.  The Israel--Stewart relaxation
  equation~\eqref{eq:rel-8-Dop} with $\Psi > 0$ converts the
  parabolic diffusion equation into a \emph{hyperbolic} (telegraphist)
  equation.  The characteristic speed of viscous signals is
  \begin{equation}
  \label{eq:rel-8-vvisc}
  v_{\mathrm{visc}} = \sqrt{\frac{\shearvisc}{\taupi\,\enthalpy_0/c^2}}
    = \sqrt{\frac{\nu_{\mathrm{rel}}}{\taupi}}
    = \frac{d}{\sqrt{\Psi}}\,\frac{\sqrt{\nu_{\mathrm{rel}}}}{d}.
  \end{equation}
  Causality requires $v_{\mathrm{visc}} \leqslant c$.  In the
  Israel--Stewart theory, $\taupi$ is bounded below by
  $\taupi \geqslant \shearvisc / (\enthalpy_0\,c^2/c^2)
  = \shearvisc\,c^2 / \enthalpy_0\,c^2 = \nu_{\mathrm{rel}}/c^2$,
  which ensures $v_{\mathrm{visc}} \leqslant c$.  This bound is
  automatically satisfied in the Israel--Stewart framework.
...
causal.  The Israel--Stewart relaxation ensures finite viscous
