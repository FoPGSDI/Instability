...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{fig_toomre_Q_disk.pdf}
\caption{%
...
%------------------------------------------------------------------
\subsection*{Deep calculation: Toomre $Q(R)$ for the Milky Way disk
  with dark matter}
\label{subsec:rel-120-deep-toomre-mw}

We compute the Toomre stability parameter across the Milky Way disk,
including stellar, gaseous, and dark-matter contributions, and assess
the magnitude of relativistic corrections near the Galactic centre.

\paragraph{Two-fluid Toomre parameter.}
For a disk containing both a stellar component (velocity dispersion
$\sigma_{\star}$, surface density $\Sigma_{\star}$) and a gas
component (sound speed $c_{s,\mathrm{gas}}$, surface density
$\Sigma_{\mathrm{gas}}$), the effective Toomre parameter is
\begin{equation}
\label{eq:rel-13-Qeff}
\frac{1}{Q_{\mathrm{eff}}}
  \approx \frac{1}{Q_{\star}} + \frac{1}{Q_{\mathrm{gas}}},
  \qquad
  Q_{\star} = \frac{\sigma_{\star}\,\kappa}{\pi G\Sigma_{\star}},
  \quad
  Q_{\mathrm{gas}} = \frac{c_{s,\mathrm{gas}}\,\kappa}{\pi G\Sigma_{\mathrm{gas}}}.
\end{equation}

\paragraph{Dark matter stabilisation.}
The dark matter halo does not participate directly in the Toomre
criterion (it has no surface density in the disk), but it maintains
the flat rotation curve ($V_{c} \approx 220\;\mathrm{km\,s^{-1}}$)
and hence the epicyclic frequency
$\kappa = \sqrt{2}\,V_{c}/R$.  Without the DM halo, the baryonic
rotation curve would be Keplerian at large $R$, reducing $\kappa$
and hence $Q$.  The DM halo therefore provides \emph{indirect
gravitational stabilisation} of the outer disk.

\paragraph{Spiral arm instability.}
In spiral arms the surface density is enhanced by a factor $\sim 2$
due to compression, driving $Q_{\mathrm{eff}}$ toward and below
unity.  This triggers gravitational fragmentation and star formation,
as observed in the molecular ring at $R \approx 4$--$5\;\mathrm{kpc}$
and in the Perseus arm~\citep{Toomre1964,BinneyTremaine2008}.

\paragraph{Relativistic correction at the Galactic centre.}
Near Sgr~A$^{*}$ ($M_{\mathrm{BH}} \approx 4 \times 10^{6}\,M_{\odot}$),
the Schwarzschild radius is $R_{S} \approx 1.2 \times 10^{12}\;\mathrm{cm}$.
At the innermost point of our disk model ($R = 0.5\;\mathrm{kpc}$),
the ratio $R_{S}/R \sim 10^{-12}$, so relativistic corrections to
$\kappa$ and $\Sigma$ are entirely negligible.  The relativistic
Toomre parameter~\eqref{eq:rel13-Qrel} reduces to the classical
expression throughout the Milky Way disk.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{fig_toomre_milky_way.pdf}
\caption{%
  \textbf{Left:} Toomre parameter $Q$ versus galactocentric radius
  for the Milky Way.  Individual components: stellar (dashed orange),
  gas (dashed green), effective two-fluid $Q_{\mathrm{eff}}$ (solid
  red), and $Q_{\mathrm{eff}}$ without the dark matter halo (dotted
  purple).  The horizontal black line marks $Q = 1$ (instability
  threshold); the solar position $R_{\odot} = 8\;\mathrm{kpc}$ is
  indicated.
  \textbf{Right:} $Q_{\mathrm{eff}}$ in the inter-arm disk (blue)
  and within spiral arms (dashed red, assuming $\times 2$
  overdensity).  Orange bands mark schematic spiral-arm crossings.
  In the inner disk ($R \lesssim 5\;\mathrm{kpc}$), spiral-arm
  compression drives $Q < 1$, triggering star formation.
}
\label{fig:toomre-mw-deep}
\end{figure}
