...
\citep{BemficaDN2018,Kovtun2019} to avoid superluminal signal
propagation.
...
In the BDNK framework \citep{BemficaDN2018,Kovtun2019}, the
...
\citep{BemficaDN2018,Kovtun2019}.  No additional relaxation-time
parameter is needed.
\end{causalitycheck}
...
\citep{BemficaDN2018,Kovtun2019}, regardless of the mode number~$j$.
...
%% =====================================================================
%%  Gavassino integration: Charge separation in accelerating plasmas
%%  Paper 44: arXiv:2508.08936 — impacts MHD equilibrium backgrounds
%% =====================================================================
\subsection{Charge separation corrections to MHD equilibria}
\label{sec:rel-10-charge-separation}

\begin{relcorrection}
\textbf{Breakdown of charge neutrality in strong gravity.}
The superposed-fluid stability analyses in this chapter (and in
Chapter~XI) assume that MHD equilibria serve as the background state.
A central assumption of standard MHD is approximate charge neutrality
($n_p \approx n_e$).  Gavassino~\cite{GavassinoChargeSeparation2025}
has shown that this assumption breaks down for relativistic plasmas
in strong gravitational fields.

For a fully ionised hydrogen plasma undergoing uniform acceleration
$g$, the equilibrium proton and electron distributions follow
Boltzmann factors that include both gravitational and electrostatic
potential energies.  The resulting charge separation is characterised
by the dimensionless ``acceleration number''
\begin{equation}
  \mathscr{A} = \frac{L_{\mathrm{Debye}}}{L_{\mathrm{accel}}}
  = \frac{\sqrt{T/(q^2 n_e)}}{T/(m_p g)}\,,
  \label{eq:rel-10-accel-number}
\end{equation}
where $L_{\mathrm{Debye}}$ is the Debye length and
$L_{\mathrm{accel}} = T/(m_p g)$ the gravitational scale height.
When $\mathscr{A} \gtrsim 1$---which can occur near black hole
horizons and in ultracompact neutron stars---the equilibrium develops
significant electric fields and charge stratification that are
absent from the standard MHD treatment.

Crucially, the equilibrium electric field is \emph{independent of
the conductivity}: it arises from the balance of gravitational and
electrostatic forces on the two species, not from Ohmic processes.
This means that the background equilibrium for stability analyses
near compact objects should, in principle, include these electric
fields.  For the oscillation modes and Rayleigh--Taylor analysis
above, the correction is negligible when $\mathscr{A} \ll 1$
(which holds for all but the most extreme environments), but it
becomes important for plasma dynamics near the innermost stable
circular orbit of accreting black holes.
\end{relcorrection}
