...
\subsection{Astrophysical application: stellar stability across
  compact-object classes}
\label{sec:rel-118-astro}

The critical adiabatic index $\gamma_{c}(\mathcal{C})$ as a function
of compactness $\mathcal{C} = GM/(Rc^{2})$ provides a unified view of
stellar stability from white dwarfs to supermassive stars.
Figure~\ref{fig:gamma-c-compactness} displays $\gamma_{c}$ across the
full range of astrophysically relevant compactnesses.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{fig_gamma_c_compactness.pdf}
\caption{%
  \textbf{Left:} Critical adiabatic index $\gamma_{c}$ versus
  stellar compactness $\mathcal{C} = GM/(Rc^{2})$ for three
  structure-dependent values of $\kappa$
  (eq.~\ref{eq:rel-13-gamma-c}).  The shaded bands mark the
  compactness ranges of white dwarfs
  ($\mathcal{C} \sim 10^{-4}$--$10^{-3}$) and neutron stars
  ($\mathcal{C} \sim 0.1$--$0.35$).  Supermassive stars (SMS) with
  radiation-dominated interiors ($\gamma_{\mathrm{eff}} \approx 4/3$)
  are driven unstable by even a tiny GR correction.
  \textbf{Right:} Normalised oscillation frequency $\omega^{2}$
  versus $\gamma$ for several compactness values, showing how the
  stability boundary shifts rightward as $\mathcal{C}$ increases.
  Configurations in the red-shaded region are dynamically unstable.
}
\label{fig:gamma-c-compactness}
\end{figure}

For \textbf{white dwarfs} ($\mathcal{C} \sim 10^{-3}$), the GR
correction $\Delta\gamma_{c} \approx 1.81 \times 10^{-3}$ is tiny
but physically decisive: it reduces the true maximum mass below the
Chandrasekhar limit~\eqref{eq:rel-13-Mch} by $\sim 0.1\%$, an effect
confirmed by full numerical integrations of the TOV
equation~\citep{ShapiroTeukolsky1983}.

For \textbf{neutron stars} ($\mathcal{C} \sim 0.15$--$0.3$), the
correction $\Delta\gamma_{c} \sim 0.3$--$0.5$ is substantial and
directly constrains the nuclear equation of state.  The requirement
that $\gamma > \gamma_{c}$ at all densities, combined with the
causality bound $c_{s} \le c$, yields the maximum neutron-star mass
$M_{\max} \approx 2$--$3\,M_{\odot}$
\citep{RezzollaZanotti2013,Lattimer2001}.

For \textbf{supermassive stars} ($M \gtrsim 10^{5}\,M_{\odot}$) with
radiation-dominated interiors ($\gamma_{\mathrm{eff}} \approx 4/3$),
even the minute GR correction triggers the post-Newtonian radial
instability first identified by Chandrasekhar (1964) and Tooper
(1966), leading to direct collapse to a black
hole~\citep{Shapiro1979,Baumgarte1999}.
...
% =====================================================================
\subsection{Deep calculation: Chandrasekhar mass limit with GR (BDNK)}
\label{sec:rel-118-deep-chandrasekhar}
% =====================================================================

We now compute the Chandrasekhar mass limit with explicit GR corrections,
following the BDNK framework for causal dissipation.

\paragraph{Electron degeneracy pressure and the Chandrasekhar EOS.}
For a white dwarf supported by electron degeneracy pressure, the
equation of state transitions from non-relativistic
($P \propto \rho^{5/3}$, $\gamma = 5/3$) to ultra-relativistic
($P \propto \rho^{4/3}$, $\gamma = 4/3$) at the relativistic
transition density
\begin{equation}
\label{eq:rel-13-rho-rel}
\rho_{\mathrm{rel}}
  = \mu_{e}\,m_{H}\,
    \frac{(2m_{e}c/\hbar)^{3}}{3\pi^{2}}
  \approx 2 \times 10^{6}\;\mathrm{g\,cm^{-3}},
\end{equation}
where $m_{e}$ is the electron mass.  The effective adiabatic index
interpolates smoothly:
\begin{equation}
\label{eq:rel-13-gamma-interp}
\gamma_{\mathrm{eff}}(\rho_{c})
  = \frac{5/3}{1 + x^{2}} + \frac{4/3\;\,x^{2}}{1 + x^{2}},
  \qquad
  x \equiv \left(\frac{\rho_{c}}{\rho_{\mathrm{rel}}}\right)^{1/3}.
\end{equation}

\paragraph{GR correction and the upper mass limit.}
From eq.~\eqref{eq:rel-13-gamma-c}, the GR correction raises the
critical index to $\gamma_{c} = 4/3 + (38/21)\,GM/(Rc^{2})$.  The
compactness of a white dwarf near the Chandrasekhar limit is
$\mathcal{C} = GM/(Rc^{2}) \sim 10^{-3}$, giving
\begin{equation}
\label{eq:rel-13-delta-gamma-wd}
\Delta\gamma_{c}
  = \frac{38}{21}\,\mathcal{C}
  \approx 1.8 \times 10^{-3}.
\end{equation}
Although small, this correction is \emph{decisive}: as
$\rho_{c} \to \infty$ the effective index
$\gamma_{\mathrm{eff}} \to 4/3$ from above, but the GR-corrected
threshold $\gamma_{c} > 4/3$.  The curves cross at a finite central
density, yielding a GR maximum mass
\begin{equation}
\label{eq:rel-13-MGR-max}
M_{\max,\mathrm{GR}}
  = M_{\mathrm{Ch}}\,\Bigl(1 - \delta\Bigr),
  \qquad
  \delta \approx \frac{1}{2}\,\kappa\,\frac{GM_{\mathrm{Ch}}}{R_{0}\,c^{2}}
  \sim 10^{-3},
\end{equation}
where $R_{0} \approx 8.7 \times 10^{8}\;\mathrm{cm}$ is the
characteristic white-dwarf radius at the mass limit.

\paragraph{BDNK dissipation.}
In the BDNK framework, the viscous terms modify the oscillation
damping rate but \emph{not} the stability boundary
(cf.~eq.~\eqref{eq:rel14-dispersion-visc} in Chapter~XIV).  The
maximum mass is therefore identical to the inviscid GR result.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{fig_chandrasekhar_limit.pdf}
\caption{%
  \textbf{Left:} Maximum white-dwarf mass as a function of central
  density $\rho_{c}$.  The Newtonian curve (blue) asymptotes to
  $M_{\mathrm{Ch}} \approx 1.44\,M_{\odot}$; the GR-corrected curve
  (red) turns over at $M_{\max,\mathrm{GR}} < M_{\mathrm{Ch}}$ and
  the configuration becomes unstable (red shading) at high density.
  The vertical dashed line marks the NR/UR transition density
  $\rho_{\mathrm{rel}}$.
  \textbf{Right:} Effective adiabatic index $\gamma_{\mathrm{eff}}$
  (blue) and GR critical index $\gamma_{c} = 4/3 +
  (38/21)\,GM/(Rc^{2})$ (red) as functions of $M/M_{\mathrm{Ch}}$.
  The crossing defines the GR instability window (red shading).
}
\label{fig:chandrasekhar-limit-deep}
\end{figure}

\bigskip
\noindent\textbf{References for \S\S\,\ref{sec:rel-116}--\ref{sec:rel-118}.}\quad
The relativistic pulsation equation and the critical adiabatic index
were derived by Chandrasekhar (1964, \emph{Ap.\ J.}\ \textbf{140},
417) and extended by Tooper (1966, \emph{Ap.\ J.}\ \textbf{143},
465).  The post-Newtonian virial theorem is due to Chandrasekhar
(1965, \emph{Ap.\ J.}\ \textbf{142}, 1488).  For comprehensive
treatments of relativistic stellar stability, see
\citet{ShapiroTeukolsky1983} and \citet{RezzollaZanotti2013}.
The maximum mass of neutron stars and the constraints from the nuclear
equation of state are reviewed in Lattimer \& Prakash (2001,
\emph{Ap.\ J.}\ \textbf{550}, 426)~\citep{Lattimer2001}.  The
post-Newtonian instability of supermassive stars is discussed in
Shapiro \& Teukolsky (1979, \emph{Ap.\ J.}\ \textbf{234},
L177)~\citep{Shapiro1979} and Baumgarte \& Shapiro (1999,
\emph{Ap.\ J.}\ \textbf{526}, 941)~\citep{Baumgarte1999}.
The entropy generation and dissipative effects in protogalactic
collapse were analysed by Weinberg (1971, \emph{Ap.\ J.}\
