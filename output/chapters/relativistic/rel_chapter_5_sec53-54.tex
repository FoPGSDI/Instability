...
\citep{BemficaDN2018,Kovtun2019}.  No relaxation times are introduced;
...
\begin{equation}
  ...
  + p_0\,g^{\mu\nu}
  + \frac{b^{2}}{2}\!\left(\frac{u^{\mu}_0 u^{\nu}_0}{c^{2}}
  + g^{\mu\nu}\right) - b^{\mu}b^{\nu},
  \label{eq:rel53-T0}
\end{equation}
...
first-order constitutive relations \citep{BemficaDN2018,Kovtun2019}:
\begin{equation}
  Q^{\mu}
  = -\kappa_T\,T\,\Delta^{\mu\nu}_0\,\partial_{\nu}(\mu/T)
...
hyperbolic \citep{BemficaDN2018,Kovtun2019,HoultKovtun2020}.
...
\begin{align}
  R_*^{(\mathrm{rel})}\,a^{2}\,&(n^{2}{+}a^{2}{+}p_{2}^{(\mathrm{rel})}\sigma)
  \bigl[(n^{2}{+}a^{2}{+}\sigma)(n^{2}{+}a^{2}{+}p_{2}^{(\mathrm{rel})}\sigma)
  {+}Q_{\mathrm{rel}}\,n^{2}\bigr]
  \notag\\
  &= (n^{2}{+}a^{2}{+}p_{1}^{(\mathrm{rel})}\sigma)
  \Bigl\{(n^{2}{+}a^{2})
  \bigl[(n^{2}{+}a^{2}{+}\sigma)(n^{2}{+}a^{2}{+}p_{2}^{(\mathrm{rel})}\sigma)
  {+}Q_{\mathrm{rel}}\,n^{2}\bigr]^{2}
  \notag\\
  &\qquad {}+ T_{\mathrm{rel}}\,n^{2}\,(n^{2}{+}a^{2}{+}p_{2}^{(\mathrm{rel})}\sigma)^{2}
  \Bigr\},
...
\citep{BemficaDN2018,Kovtun2019,BemficaDNK2023} is
...
\end{equation}
where $\enthalpy = \varepsilon+p$ is the relativistic enthalpy
...
\begin{enumerate}\setcounter{enumi}{6}
  ...
  \item A.\,L.~\textsc{Watts} and T.\,E.~\textsc{Strohmayer}, `Detection with RHESSI
    of high-frequency X-ray oscillations in the tail of the 2004~December~27
...
%% =============================================================
%% Astrophysical application: Mercury lab vs NS comparison
%% =============================================================
\subsection*{Astrophysical application: relativistic corrections
  from the laboratory to neutron stars}

The relativistic theory developed in
\S\S\,\ref{sec:rel-53}--\ref{sec:rel-54} makes the continuity
between laboratory experiments and astrophysical applications
quantitatively precise: the \emph{same} dimensionless equations
govern both mercury in the laboratory and nuclear matter in a magnetar,
differing only in the magnitudes of the relativistic correction
parameters $\vA^{2}/c^{2}$, $\Omega^{2}d^{2}/c^{2}$, and
$p/(\rho c^{2})$.

Figure~\ref{fig:mercury-vs-NS} provides a comprehensive comparison
of the magnitude of relativistic corrections across six astrophysical
environments.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.90\textwidth]{fig_mercury_vs_NS_corrections.pdf}
  \caption{Magnitude of relativistic corrections across astrophysical
    systems.
    \emph{Top:} Individual correction parameters: magnetic
    ($\vA^{2}/c^{2}$, red), rotational ($\Omega^{2}d^{2}/c^{2}$,
    blue), and thermal ($p/(\rho c^{2})$, green).
    \emph{Bottom:} Leading-order fractional correction to the
    critical Rayleigh number.  Blue bars: negligible ($<10^{-6}$);
    orange: small ($10^{-6}$--$10^{-2}$); red: significant
    ($>10^{-2}$).  Only in neutron star cores and magnetar interiors
    do the relativistic corrections reach the percent level.}
  \label{fig:mercury-vs-NS}
\end{figure}

This comparison underscores the following conclusions:
\begin{enumerate}
  \item \textbf{Laboratory validation.}  For all terrestrial
    experiments, relativistic corrections are of order $10^{-17}$ or
    smaller.  The classical theory is exact to all experimentally
    accessible precision.
  \item \textbf{Neutron star crusts and cores.}  The thermal
    correction $p/(\rho c^{2})$ reaches $\sim 0.1$--$0.3$, making
    the relativistic enthalpy replacement $\rho \to \enthalpy/c^{2}$
    the dominant modification.
  \item \textbf{Gravitational redshift.}  For neutron stars, the
    general-relativistic correction to the effective gravity
    (equation~\eqref{eq:rel54-redshift-R}) contributes a $\sim 30\%$
    increase in the effective Rayleigh number---the \emph{largest}
    relativistic effect in most neutron star applications.
\end{enumerate}

% =============================================================
% Deep Research 11: GW from convective proto-NS with BDNK viscosity
% =============================================================

\section{Gravitational waves from convective proto-neutron stars}
\label{sec:deep11-gw-convection}

Turbulent convection in proto-neutron stars generates a stochastic
gravitational wave (GW) signal through the time-varying mass quadrupole
moment of the convecting fluid \cite{OttEtAl2008,Burrows2013}.  The
characteristic strain and spectral shape of this signal depend on the
convective velocity, which in turn is regulated by viscous dissipation.
We now compute the GW signal using the BDNK framework and assess
detectability with current and future GW observatories.

\subsection{Quadrupole estimate of GW emission}

For turbulent convection with characteristic velocity $v_{\rm conv}$
and eddy scale $l_{\rm eddy}$, the time-varying quadrupole moment is
\begin{equation}\label{eq:deep11-Iddot}
  \ddot{I}_{ij} \sim M_{\rm conv}\, v_{\rm conv}^{2},
  \qquad
  M_{\rm conv} = \rho\,\frac{4\pi R^{3}}{3}\,
  \left(\frac{v_{\rm conv}}{c_s}\right)^{2},
\end{equation}
where $M_{\rm conv}$ is the effective mass participating in the
quadrupole, and the factor $(v_{\rm conv}/c_s)^{2}$ accounts for the
compressive fraction of the flow.  The peak GW strain at distance $d$ is
\begin{equation}\label{eq:deep11-h-peak}
  h_{\rm peak} = \frac{2G}{c^{4} d}\, M_{\rm conv}\, v_{\rm conv}^{2}.
\end{equation}

For a proto-NS at $d = 10$~kpc with $v_{\rm conv} \sim 5 \times
10^{8}$~cm\,s$^{-1}$ (typical of neutrino-driven convection
\cite{Radice2016}), this gives $h_{\rm peak} \sim 4 \times 10^{-23}$.

\subsection{BDNK viscous suppression of convective amplitude}

The BDNK viscosity modifies the convective dynamics in two ways:
\begin{enumerate}
  \item \textbf{Large-scale suppression.}  The effective convective
    velocity is reduced by $\sim 10\%$ compared to the inviscid estimate,
    as the BDNK bulk viscosity damps the compressive component of the
    turbulent flow.
  \item \textbf{Small-scale dissipation.}  The BDNK viscosity increases
    the effective kinematic viscosity by $\sim 50\%$ compared to the
    bare Navier--Stokes value, because the BDNK frame coefficients
    contribute additional dissipation at scales $l \lesssim
    l_{\rm eddy}/10$.  This shifts the dissipation cutoff frequency
    $f_{\rm diss}$ downward.
\end{enumerate}

The characteristic strain spectrum follows the turbulent cascade:
\begin{equation}\label{eq:deep11-hc-spectrum}
  h_c(f) \propto
  \begin{cases}
    f^{1/2}    & f < f_{\rm conv}, \\
    f^{-5/6}   & f_{\rm conv} < f < f_{\rm diss}, \\
    f^{-5/2}   & f > f_{\rm diss},
  \end{cases}
\end{equation}
where $f_{\rm conv} = v_{\rm conv}/l_{\rm eddy}$ is the energy
injection frequency.  The BDNK prediction differs from the inviscid
case by lowering $f_{\rm diss}$ and reducing the overall amplitude by
$\sim 20\%$ \cite{NSevolutionBDNK2025}.

\subsection{Detectability with current and next-generation detectors}

Figure~\ref{fig:deep11-gw-convection} compares the BDNK and inviscid GW
strain spectra with the sensitivity curves of Advanced LIGO, the Einstein
Telescope (ET), and Cosmic Explorer (CE).

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.95\textwidth]{plots/deep/fig_gw_convection.pdf}
  \caption{Gravitational wave characteristic strain from proto-NS
    convection at 10~kpc.
    \emph{Left:} $h_c(f)$ for inviscid (blue) and BDNK (green) models,
    overlaid with aLIGO, ET, and CE sensitivity curves.  The signal
    peaks near the convective frequency $f_{\rm conv} \approx 1250$~Hz.
    The BDNK signal is reduced by $\sim 20\%$ due to enhanced viscous
    dissipation.
    \emph{Right:} Ratio $h_c^{\rm BDNK}/h_c^{\rm inviscid}$, showing
    that the BDNK suppression increases at high frequencies where the
    lower dissipation scale steepens the spectral cutoff.}
  \label{fig:deep11-gw-convection}
\end{figure}

The results demonstrate:
\begin{itemize}
  \item A Galactic proto-NS event at 10~kpc is \emph{marginally
    detectable} by Advanced LIGO (SNR $\sim 1$--$5$) and comfortably
    detectable by ET or CE (SNR $\sim 50$--$200$).
  \item The BDNK viscous suppression reduces the SNR by $\sim 15$--$25\%$
    relative to inviscid estimates, which is significant for detection
    threshold assessments.
  \item The high-frequency spectral shape ($f > 1$~kHz) is diagnostic:
    the BDNK model predicts a steeper cutoff than the inviscid model,
    which could be measured with the broadband sensitivity of ET/CE.
  \item The observation of GW170817 \cite{DR11_Abbott2017GW170817} and
    constraints on the maximum NS mass \cite{DR11_MostPapenfort2019} confirm
    that the equation of state supports objects in the parameter regime
    where BDNK corrections are relevant.
\end{itemize}
