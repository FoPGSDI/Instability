...
% ===================================================================
\section{Deep Research: direct imaging of KH vortices in 3C\,84}
\label{sec:rel-11-104-3c84}

A remarkable observational confirmation of the relativistic KH
instability in jets has recently been achieved by \cite{Fuentes2025},
who report the direct imaging of KH vortices in the jet of the radio
galaxy 3C\,84 (NGC\,1275, Perseus\,A).

\subsection{Observational context}
\label{sec:rel-11-104-3c84-obs}

3C\,84 is a nearby ($z = 0.0176$) Fanaroff--Riley type~I radio galaxy
hosting a prominent two-sided jet launched from a supermassive black
hole of mass $M \sim 9 \times 10^{8}\,M_{\odot}$.  High-resolution
VLBI observations at 22 and 43\,GHz reveal a mildly relativistic jet
with $\Gamma \approx 2$--$3$, an opening angle of $\sim 1$--$2^{\circ}$
at sub-parsec scales, and a prominent limb-brightened morphology
\cite{Fuentes2025}.

The key discovery is the detection of vortex-like structures at
$\sim 0.3\,$pc from the core, with morphology and kinematics
consistent with the nonlinear saturation of KH surface modes.
These vortices have a characteristic transverse scale comparable to
the jet radius and propagate downstream at $\sim 0.3\,c$, matching
the predicted phase velocity of the $m=1$ helical KH mode.

\subsection{Relativistic Richardson number for 3C\,84}
\label{sec:rel-11-104-3c84-ri}

Using the observed parameters ($\Gamma \approx 2.5$,
$\eta = \hat{\rho}_{\mathrm{jet}}/\hat{\rho}_{\mathrm{amb}} \approx 0.05$,
shear-layer width $d \sim 0.1\,$pc), the relativistic Richardson
number~\eqref{eq:rel-104-11} across the jet shear layer is
\begin{equation}\label{eq:rel-104-3c84-1}
  J_{\mathrm{rel}}
  = \frac{J_{\mathrm{class}}}{\Gamma_{0}^{4}}
  \approx \frac{J_{\mathrm{class}}}{39}\,.
\end{equation}
For the observed density gradient $\Delta\hat{\rho}/\hat{\rho} \sim 0.95$
across the shear layer, the classical Richardson number is
$J_{\mathrm{class}} \approx 3$--$5$.  The relativistic suppression
factor $\Gamma^{-4} \approx 0.026$ drives
$J_{\mathrm{rel}} \approx 0.08$--$0.13$, well below the Miles--Howard
threshold $J_{\mathrm{rel}} = 1/4$.

This quantitative assessment confirms that the 3C\,84 jet shear layer
is KH-unstable, in agreement with the observed vortex structures.
The classical Richardson number alone would predict marginal stability
($J_{\mathrm{class}} \gg 1/4$); it is the relativistic $\Gamma^{-4}$
suppression that drives the layer into the unstable regime, providing
a textbook demonstration of the relativistic enhancement of KH
instability.

\subsection{Comparison with the Drazin marginal curve}
\label{sec:rel-11-104-3c84-drazin}

The stability boundary~\eqref{eq:rel-104-17} predicts that, for
$\Gamma = 2.5$ ($V_{0}/c = 0.92$), the relativistic correction to
the marginal Richardson number is
\begin{equation}
  J_{\mathrm{rel,crit}}^{(\mathrm{max})}
  = \frac{1}{4}\!\left(1 + \frac{0.92^{2}}{3}\right)
  \approx 0.32\,,
\end{equation}
which is about $30\%$ above the classical value of $1/4$.  Even with
this upward shift of the instability boundary, the 3C\,84 shear
layer ($J_{\mathrm{rel}} \approx 0.1$) lies well within the
unstable region.

Figure~\ref{fig:3c84-kh-vortex} shows the KH stability map in the
$(\Gamma, \eta)$ plane with 3C\,84, M87, and Cen~A marked, together
with the Richardson number profile across the 3C\,84 shear layer.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\textwidth]{fig_3c84_kh_vortex.pdf}
\caption{%
\textbf{Left:} KH stability map in the $(\Gamma, \eta)$ plane,
where $\eta = \hat{\rho}_{\mathrm{jet}}/\hat{\rho}_{\mathrm{amb}}$.
The colour scale shows the normalised growth rate; the white dashed
contour marks the Miles--Howard boundary
$\mathrm{Ri}_{\mathrm{rel}} = 1/4$.  Symbols mark 3C\,84
\citep{Fuentes2025}, M87 \citep{WalkerEtAl2018}, and Cen~A.
\textbf{Right:} Richardson number profile across the 3C\,84 shear
layer ($\Gamma = 2.5$); the red shaded region marks the
KH-unstable zone ($\mathrm{Ri}_{\mathrm{rel}} < 1/4$).
The purple band indicates the observed vortex location.
}
\label{fig:3c84-kh-vortex}
\end{figure}
